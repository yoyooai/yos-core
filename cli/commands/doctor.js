/**
 * yos doctor — diagnose installation health and auto-fix via Claude.
 *
...
export function buildDiagnosticJson(diag, coreVersion) {
  ...
  if (!diag.system.tmux.installed) {
    issues.push({ id: 'tmux_missing', label: 'tmux not installed', hint: 'Run yos init' });
  }
  if (!diag.system.pm2.installed) {
    issues.push({ id: 'pm2_missing', label: 'PM2 not installed', hint: 'Run yos init' });
  }
  ...
  if (!diag.ai.cli.installed) {
    issues.push({ id: 'cli_missing', label: `${runtimeLabel} CLI not installed`, hint: 'Run yos init' });
  }
  if (diag.ai.cli.installed && !diag.ai.networkSkipped && diag.ai.authStatus === 'failure') {
    const hint = ACTIVE_RUNTIME === 'codex' ? 'Run: codex login' : 'Run yos init to authenticate';
  ...
  if (diag.system.pm2.installed) {
    if (!diag.services.running || diag.services.total === 0) {
      issues.push({ id: 'services_down', label: 'PM2 services not started', hint: 'Run: pm2 start ~/yos/pm2/ecosystem.config.cjs && pm2 save' });
    } else {
...
function discoverChannels(pm2Procs, env, tmuxSession, components) {
  ...
  if (webConsole) {
    ...
    if (caddyOnline) {
      ...
      hint = `Tip: ${bold('yos init')} can set up a custom domain for remote access.`;
    }
...
function runClaudeFix(diagnosticJson) {
  ...
  const prompt = [
    `You are a YOS repair agent. Working directory: ${YOS_DIR}`,
    '',
    'Diagnostic report from `yos doctor --json`:',
    JSON.stringify(diagnosticJson, null, 2),
    '',
    'Fix all issues listed above:',
    issueList,
    '',
    'After fixing, verify by running: yos doctor --json',
...
export async function doctorCommand(args) {
  ...
  if (initMarkersFound === 0) {
    if (jsonMode) {
      console.log(JSON.stringify({ passed: false, error: 'not_initialized', hint: 'Run yos init' }));
      process.exit(1);
    }
    console.log(`\n${yellow('YOS has not been initialized yet.')}\n`);
    console.log(`  Run ${bold('yos init')} to set up your environment.`);
    ...
    logToFile('result: not initialized — suggested yos init');
    process.exit(1);
  }

  if (initMarkersFound < initMarkers.length) {
    if (jsonMode) {
      console.log(JSON.stringify({ passed: false, error: 'incomplete_init', hint: 'Run yos init' }));
      process.exit(1);
    }
    console.log(`\n${yellow('YOS initialization appears incomplete.')}\n`);
    console.log(`  Run ${bold('yos init')} to complete the setup.\n`);
    logToFile('result: incomplete init — suggested yos init');
    process.exit(1);
  }
  ...
  if (!jsonMode) {
    ...
    console.log(`\n${heading('Checking your YOS setup...')}${versionLabel}\n`);
  }
  ...
  if (procs.length > 0 || diag.services.session) {
    ...
    if (offlineChannels.length > 0) {
      if (onlineChannels.length > 0) {
        ...
        console.log(`\n  ${dim('Run')} ${bold('yos init')} ${dim('to set up channels.')}`);
      }
    }
  }
  ...
  if (diag.system.network.reachable) {
    try {
      ...
      if (coreVersion.success && releaseSource.success) {
        targets.push({ name: 'YOS core', repo: releaseSource.repo, current: coreVersion.version });
      }
      ...
      if (updates.length > 0) {
        ...
        console.log(`\n  ${dim("Run")} ${bold('yos upgrade --all')} ${dim('to update.')}`);
      }
    } catch {}
  }
  ...
  if (diagnostic.passed) {
    ...
    if (sessionStarting) {
      console.log(`\n  ${dim('Claude session is starting — run')} ${bold('yos attach')} ${dim('in a moment to connect.')}`);
    }
    console.log('');
  ...
  if (claudeReady) {
    ...
    if (reverify.passed) {
      ...
      console.log(`\n${green('✓ All issues fixed — everything is working. Enjoy your YOS!')}`);
      if (sessionStarting) {
        console.log(`\n  ${dim('Claude session is starting — run')} ${bold('yos attach')} ${dim('in a moment to connect.')}`);
      }
      console.log('');
  ...
  if (!diag.system.network.reachable) {
    console.log(`\n  ${yellow('Check your network connection and run')} ${bold('yos doctor')} ${yellow('again.')}\n`);
  } else {
    console.log(`\n  ${yellow('Run')} ${bold('yos init')} ${yellow('to set up your environment.')}\n`);
  }
