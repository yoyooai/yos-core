...
import { YOS_DIR, SKILLS_DIR, getYosConfig } from '../lib/config.js';
...
export async function showStatus() {
  console.log(heading('YOS Status') + '\n' + dim('============') + '\n');
  ...
  if (!commandExists(runtimeBin)) {
    ...
    if (authStatus === 'failure') {
      ...
      console.log(`  ${dim(`→ Run: ${isCodex ? 'codex login' : 'yos init to authenticate'}`)}`);
    } else if (authStatus === 'uncertain') {
      console.log(`${bold(runtimeLabel)}: ${yellow('AUTH CHECK INCONCLUSIVE')}`);
      console.log(`  ${dim('→ Retry later or run yos doctor for details')}`);
    } else if (!isCodex) {
      ...
      if (!termsAccepted) {
        ...
        console.log(`  ${dim('→ Run: yos init to complete setup')}`);
      } else {
        // Check runtime status from activity monitor
        const statusFile = path.join(YOS_DIR, 'activity-monitor', 'agent-status.json');
  ...
  try {
    const df = execSync(`df -h ${YOS_DIR} | tail -1`, { encoding: 'utf8' });
...
export function startServices() {
  console.log(heading('Starting YOS services...'));
  ...
  if (fs.existsSync(ecosystemPath)) {
    try {
      ...
      console.log(`\n${green('Services started.')} Run ${dim('"yos status"')} to check.`);
      return;
    } catch (e) {
  ...
  if (started > 0) {
    execSync('pm2 save 2>/dev/null', { stdio: 'pipe' });
    console.log(`\n${green(started + ' services started.')} Run ${dim('"yos status"')} to check.`);
  } else {
...
export function stopServices() {
  console.log(heading('Stopping YOS services...'));
  const services = ['activity-monitor', 'scheduler', 'c4-dispatcher', 'web-console'];
  try {
