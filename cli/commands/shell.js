/**
 * yos shell — CLI interactive mode
 *
...
const C4_RECEIVE = path.join(YOS_DIR, '.claude', 'skills', 'comm-bridge', 'scripts', 'c4-receive.js');

export async function shellCommand() {
  const socketPath = path.join(os.tmpdir(), `yos-shell-${process.pid}.sock`);
  ...
  if (!fs.existsSync(C4_RECEIVE)) {
    console.error('Error: comm-bridge not found. Run "yos doctor" to check your installation.');
    process.exit(1);
  }
...
function cleanStaleSockets() {
  ...
  try {
    ...
    for (const file of files) {
      const match = file.match(/^yos-shell-(\d+)\.sock$/);
      if (!match) continue;
...
function printHelp() {
  console.log(`
${bold('YOS Shell')} — Interactive CLI
