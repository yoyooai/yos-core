...
import { YOS_DIR, SKILLS_DIR, COMPONENTS_DIR, getYosConfig } from '../lib/config.js';
...
export function formatC4Reply(type, data) {
  switch (type) {
    ...
    case 'self-check': {
      ...
      if (!hasUpdate) return `yos-core is up to date (v${current})`;
      let r = `yos-core: ${current} -> ${latest}`;
      if (changelog) r += `\n\nChangelog:\n${changelog}`;
      ...
      r += '\n\nReply "upgrade yos confirm" to proceed.';
      return r;
    }
    case 'upgrade': {
    ...
    case 'self-upgrade': {
      ...
      let r = `yos-core upgraded: ${from} -> ${to}`;
      if (changelog) r += `\n\nChangelog:\n${changelog}`;
      if (mergedFiles?.length > 0) {
        r += `\n\nAuto-merged files: ${mergedFiles.join(', ')}`;
      }
      ...
      if (migrationHints?.length > 0) {
        r += '\n\nACTION REQUIRED - Hook changes in ~/yos/.claude/settings.json:';
        ...
        r += '\nPlease update hooks in ~/yos/.claude/settings.json and restart Claude to apply.';
      }
...
export async function upgradeComponent(args) {
  ...
  // Validate target
  if (!target) {
    console.error('Usage: yos upgrade <name> [options]');
    console.error('       yos upgrade --all');
    console.error('       yos upgrade --self');
    console.log('\nOptions:');
...
async function handleCheckOnly(component, { jsonOutput, branch, beta = false }) {
  ...
  if (jsonOutput) {
    ...
    if (!result.hasUpdate && !branch) {
      ...
      console.log(`\n${dim(`Run "yos upgrade ${component} --yes" to upgrade.`)}`);
    } else {
...
async function upgradeAllComponents({ checkOnly, jsonOutput, skipConfirm, skipEval, beta = false, mode = 'merge' }) {
  ...
  if (checkOnly) {
    console.log(dim('Run "yos upgrade --all --yes" to upgrade all.'));
    return;
  }
...
function handleSelfCheckOnly({ jsonOutput, branch, beta = false }) {
  ...
  if (!check.success) {
    if (jsonOutput) {
      const errOutput = { action: 'check', target: 'yos-core', ...check };
      errOutput.reply = formatC4Reply('error', check);
      console.log(JSON.stringify(errOutput, null, 2));
    } else {
      console.error(`Error: ${check.message}`);
...
async function upgradeSelfCore({ branch, beta = false, mode = 'merge' } = {}) {
  ...
  try {
    ...
    // 3. Download new version to temp (always fresh in confirm/--yes flow)
    const downloadLabel = branch ? `yos-core (branch: ${branch})` : `yos-core@${check.latest}`;
    if (!jsonOutput) {
      console.log(`\n${dim('Confirm flow uses a fresh download (check artifacts are not reused).')}`);
      console.log(`Downloading ${bold(downloadLabel)}...`);
    }
    ...
    if (!jsonOutput) {
      console.log(`\n${bold('YOS core')}: ${dim(check.current)} → ${bold(check.latest)}`);
      const targetVersion = check.latest || (branch ? `branch:${branch}` : 'unknown');
      const targetRef = branch ? `branch:${branch}` : `tag:v${check.latest}`;
      console.log(dim(`Target package: ${targetVersion} (${targetRef})`));
    ...
    if (!skipConfirm) {
      ...
      console.log(`Upgrading ${bold('YOS core')}...`);
    }
    ...
    if (jsonOutput) {
      ...
      if (result.success && result.instructionFilesRebuilt) {
        try {
          const activeRuntime = getYosConfig().runtime ?? 'claude';
          if (activeRuntime === 'claude') {
            const c4ControlPath = path.join(YOS_DIR, '.claude', 'skills', 'comm-bridge', 'scripts', 'c4-control.js');
      ...
      if (result.migrationHints?.length > 0) {
        ...
        console.log(`\nUpdate hooks in ${bold('~/yos/.claude/settings.json')} and restart Claude to apply.`);
      }
...
export async function uninstallComponent(args) {
  // yos uninstall --self → full system uninstall
  if (args.includes('--self')) {
    const hasTarget = args.some(a => !a.startsWith('-'));
    if (hasTarget) {
      console.error('Error: --self cannot be combined with a component name.');
      console.error('  To uninstall a component:  yos uninstall <name>');
      console.error('  To uninstall YOS itself: yos uninstall --self');
      process.exit(1);
    }
  ...
  if (!target) {
    console.error('Usage: yos uninstall <name> [options]');
    console.log('\nOptions:');
    ...
    console.log('  --self     Uninstall YOS entirely from the system');
    process.exit(1);
  }
...
/**
 * Resolve PM2 service name from SKILL.md or fallback to yos-<name>.
 */
function resolveServiceName(name) {
  const skillDir = path.join(SKILLS_DIR, name);
  const skill = parseSkillMd(skillDir);
  return skill?.frontmatter?.lifecycle?.service?.name || `yos-${name}`;
}
...
function handleUninstallCheck(target, { jsonOutput }) {
  ...
  if (jsonOutput) {
    ...
    console.log(`\n${dim(`Run "yos uninstall ${target} --yes" to proceed.`)}`);
  }
}
    ...
    if (fs.existsSync(hookPath)) {
      ...
      try {
        execFileSync('node', [hookPath], {
          cwd: skillDir,
          stdio: jsonOutput ? 'pipe' : 'inherit',
          env: { ...process.env, YOS_COMPONENT: target, YOS_SKILL_DIR: skillDir, YOS_DATA_DIR: dataDir },
        });
...
export async function infoComponent(args) {
  ...
  if (!target) {
    console.error('Usage: yos info <name> [--json]');
    process.exit(1);
  }
  ...
  const serviceName = fm.lifecycle?.service?.name || `yos-${target}`;
...
export async function listComponents() {
  ...
  if (names.length === 0) {
    console.log('No components installed.');
    console.log(`\n${dim('Use "yos search <keyword>" to find available components.')}`);
    console.log(dim('Use "yos add <name>" to install a component.'));
    return;
  }
...
export async function searchComponents(args) {
  ...
  if (results.length === 0) {
    ...
    if (keyword) {
      ...
      console.log(dim('  yos add <github-url>'));
    }
    return;
  }
  ...
  console.log(`\n${dim('Use "yos add <name>" to install a component.')}`);
}
