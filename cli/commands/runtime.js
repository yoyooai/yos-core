/**
 * yos runtime — switch agent runtime without re-running full init.
 *
 * Usage:
 *   yos runtime <name>                        Switch to the specified runtime (claude|codex)
 *   yos runtime status                        Show the currently configured runtime
 *   yos runtime <name> --save-apikey <key>    Save API key and switch (all runtimes)
 *   yos runtime <name> --save-base-url <url>  Save base URL and switch
 *   yos runtime claude --save-setup-token <t> Save Claude setup token and switch
...
import { getYosConfig, updateYosConfig, YOS_DIR } from '../lib/config.js';
...
/**
 * Entry point for `yos runtime [subcommand|name]`.
 *
...
export function prepareRuntimeInstruction(target, { yosDir = YOS_DIR } = {}) {
  const instructionPath = buildInstructionFile(target, { yosDir });
...
export function applyBaseUrl(target, baseUrl) {
  ...
  if (target === 'codex') {
    ...
    return writeCodexConfig(YOS_DIR, { openaiBaseUrl: baseUrl });
  }
  return false;
}
...
export function validateRuntimeFlags(target, parsed) {
  if (parsed.hasApiKey && (!parsed.apiKey || parsed.apiKey.startsWith('--'))) {
    return {
      ...
      example: target === 'codex'
        ? 'yos runtime codex --save-apikey sk-proj-xxx'
        : 'yos runtime claude --save-apikey sk-ant-api-xxx',
    };
  }
  if (parsed.hasSetupToken && (!parsed.setupToken || parsed.setupToken.startsWith('--'))) {
    return {
      error: 'Missing value for --save-setup-token.',
      example: 'yos runtime claude --save-setup-token sk-ant-oat-xxx',
    };
  }
  if (parsed.hasBaseUrl && (!parsed.baseUrl || parsed.baseUrl.startsWith('--'))) {
    return {
      ...
      example: target === 'codex'
        ? 'yos runtime codex --save-base-url https://proxy.example.com/v1'
        : 'yos runtime claude --save-base-url https://claude-proxy.example.com',
    };
  }
...
export async function checkRuntimeAuthGate(target, adapter, parsed, {
  ...
  if (target === 'codex') {
    error(`  ${cyan('yos runtime codex --save-apikey <key>')}     ${dim('OpenAI API key')}`);
    ...
    error(`  ${cyan('yos runtime claude --save-apikey <key>')}       ${dim('Anthropic API key (sk-ant-api...)')}`);
    error(`  ${cyan('yos runtime claude --save-setup-token <token>')} ${dim('Setup token (sk-ant-oat...)')}`);
...
async function switchRuntime(target, flags) {
  const cfg = getYosConfig();
  const current = cfg.runtime ?? 'claude';
  const monitorDir = path.join(YOS_DIR, 'activity-monitor');
  ...
  // Step 4: Prepare the target runtime before changing persistent state.
  console.log(`Rebuilding instruction file for ${bold(target)}...`);
  try {
    prepareRuntimeInstruction(target);
    console.log(`  ${green('✓')} done`);
  } catch (e) {
    console.error(`  ${red(`Runtime switch aborted: ${e.message}`)}`);
    console.error(`  ${dim('Use a fresh YOS installation with the current instruction layout.')}`);
    process.exitCode = 1;
    return;
  }

  // Step 5: Persist only after the target runtime is ready.
  updateYosConfig({ runtime: target });
  ...
  // session (e.g. via "yos attach"), and killing its own parent session would terminate
...
export function showHelp() {
  console.log(`
yos runtime — switch agent runtime

Usage:
  yos runtime <name>                        Switch to the specified runtime
  yos runtime status                        Show currently configured runtime
  yos runtime <name> --save-apikey <key>    Save API key and switch
  yos runtime <name> --save-base-url <url>  Save base URL and switch
  yos runtime claude --save-setup-token <t> Save Claude setup token and switch
  yos runtime <name> --no-validate          Skip switch-time auth probe
...
Examples:
  ...
  yos runtime claude --save-apikey sk-ant-api-xxx
  yos runtime claude --save-base-url https://claude-proxy.example.com
  yos runtime claude --save-setup-token sk-ant-oat-xxx
  yos runtime codex --save-apikey sk-proj-xxx
  yos runtime codex --save-base-url https://proxy.example.com/v1
  yos runtime codex --no-validate
`);
}
