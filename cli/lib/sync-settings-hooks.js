...
import { getYosConfig, updateYosConfig } from './config.js';
...
const YOS_DIR = path.resolve(process.env.YOS_DIR || path.join(os.homedir(), 'yos'));
const TEMPLATE_SETTINGS = path.join(__dirname, '..', '..', 'templates', '.claude', 'settings.json');
const INSTALLED_SETTINGS = path.join(YOS_DIR, '.claude', 'settings.json');
...
export const CORE_MANAGED_HOOKS = new Set([
  ...
  'skills/yos-memory/scripts/session-start-inject.js',
...
export function isCoreManaged(hook, { yosDir = YOS_DIR } = {}) {
  ...
  return CORE_MANAGED_HOOKS.has(key)
    || key === path.resolve(yosDir, '.yos', 'instructions', 'assembler.mjs').replaceAll('\\', '/');
}

function yosClaudeScript(relativePath) {
  const scriptPath = path.resolve(YOS_DIR, '.claude', relativePath);
...
function commandHook(relativePath, options = {}) {
  return {
    type: 'command',
    command: yosClaudeScript(relativePath),
    ...options,
  };
}
...
export function desiredSessionStartHooks({
  ...
  const orchestrator = yosClaudeScript('skills/activity-monitor/scripts/session-start-orchestrator.js');
  const instructionsDir = path.resolve(yosDir, '.yos', 'instructions');
  const assembler = path.join(instructionsDir, 'assembler.mjs');
  const assemblerHook = canonicalAssemblerEntry({ yosDir });
  const hooks = names.map(name => ({
    type: 'command',
    command: `${orchestrator} --shard ${name}`,
    timeout: 20000,
  }));
  // postinstall can run before `yos init` materializes the assembler.
...
export function canonicalAssemblerEntry({ yosDir = YOS_DIR } = {}) {
  ...
  return {
    ...
    command: [
      ...
      '--user', JSON.stringify(path.join(root, 'YOS.md')),
 ...
 */
export function reconcileAssemblerEntries(settings, { yosDir = YOS_DIR } = {}) {
  const canonical = canonicalAssemblerEntry({ yosDir });
...
export function desiredClaudeHooks({
  ...
  const sessionStartHooks = desiredSessionStartHooks({ yosDir, existsSync });
 ...
 * may claim its own old hook paths (relative to ~/yos/.claude, enforced at
 * declaration validation), and everything unclaimed — user hooks, undeclared
 * component hooks, anything outside the yos .claude root — is preserved.
 */
export function claimedHookBaseKeys({ yosDir = YOS_DIR } = {}) {
  ...
  for (const [event, matchers] of Object.entries(installedSettings.hooks)) {
    ...
    for (let gi = matchers.length - 1; gi >= 0; gi--) {
      ...
      for (let hi = group.hooks.length - 1; hi >= 0; hi--) {
        ...
        // and only via relative-to-.claude keys, so hooks outside the yos
        // root can never match a claim.
        const claimed = event === 'SessionStart'
          && claimedKeys.size > 0
          && claimedKeys.has(hookScriptBaseKey(h.command));
        if (!isCoreManaged(h, { yosDir }) && !claimed) continue;
        ...
        if (!foundInTemplate) {
          ...
          removed++;
          log(`  - ${event}[${groupMatcher}]: ${h.command}${claimed && !isCoreManaged(h, { yosDir }) ? ' (claimed by component shard declaration)' : ''}`);
        }
      }
