 ...
 * e.g. "node ~/yos/.claude/skills/foo/scripts/bar.js --flag"
 *   -> "/home/user/yos/.claude/skills/foo/scripts/bar.js"
...
function yosClaudeRoot() {
  const yosDir = path.resolve(process.env.YOS_DIR || path.join(os.homedir(), 'yos'));
 ...
 * "is this a yos-core-managed script" checks (CORE_MANAGED_HOOKS lists bare
 * script paths, and every `--shard` variant of a core script is core-managed).
 * Only hooks under the yos-owned .claude directory are reduced to registry
