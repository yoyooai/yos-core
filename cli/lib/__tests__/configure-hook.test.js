...
describe('configure-hook', () => {
  ...
  it('pipes collected config as stdin JSON with component env vars', () => {
    ...
    fs.writeFileSync(path.join(hookDir, 'configure.js'), `
      ...
      fs.writeFileSync(process.env.YOS_DATA_DIR + '/captured.json', JSON.stringify({
        component: process.env.YOS_COMPONENT,
        skillDir: process.env.YOS_SKILL_DIR,
        dataDir: process.env.YOS_DATA_DIR,
