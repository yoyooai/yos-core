...
describe('loadRuntimeEnvManifest', () => {
  it('loads and parses manifest from .yos/ subdir', () => {
    ...
    fs.writeFileSync(path.join(yosSubdir, 'runtime-env.manifest'), 'env MY_VAR\npath_prepend /x');
...
describe('buildCleanEnv', () => {
  ...
  it('injects YOS_TMUX_ENV vars from dotenvVars', () => {
    const dotenvVars = {
      YOS_TMUX_ENV: 'MY_VAR,OTHER_VAR',
      MY_VAR: 'value1',
      OTHER_VAR: 'value2',
    };
    const { env } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
  ...
  it('injects YOS_TMUX_INHERIT vars from processEnv', () => {
    const processEnv = { ...baseProcessEnv, SSH_AUTH_SOCK: '/tmp/ssh.sock' };
    const dotenvVars = { YOS_TMUX_INHERIT: 'SSH_AUTH_SOCK' };
    const { env } = buildCleanEnv({ processEnv, dotenvVars, platform: 'linux' });
    assert.equal(env.SSH_AUTH_SOCK, '/tmp/ssh.sock');
  });

  it('YOS_TMUX_ENV wins over YOS_TMUX_INHERIT on conflict', () => {
    const processEnv = { ...baseProcessEnv, CONFLICT_VAR: 'from_process' };
    const dotenvVars = {
      YOS_TMUX_ENV: 'CONFLICT_VAR',
      YOS_TMUX_INHERIT: 'CONFLICT_VAR',
  ...
  it('PREPEND inserts before platform/system base paths', () => {
    const dotenvVars = { YOS_TMUX_PATH_PREPEND: '/custom/tools,/extra/bin' };
    const { env } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
    const parts = env.PATH.split(':');
  ...
  it('APPEND inserts after system base paths', () => {
    const dotenvVars = { YOS_TMUX_PATH_APPEND: '/late/tools' };
    const { env } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
    const parts = env.PATH.split(':');
  ...
  it('relative path in PREPEND is skipped with warning', () => {
    const dotenvVars = { YOS_TMUX_PATH_PREPEND: '/ok,relative/bad' };
    const { env, warnings } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
    const parts = env.PATH.split(':');
    assert.ok(parts.includes('/ok'));
    assert.ok(!parts.includes('relative/bad'));
    assert.ok(warnings.some(w => w.includes('YOS_TMUX_PATH_PREPEND') && w.includes('relative/bad')));
  });
  ...
  it('empty PATH manifest does not change PATH', () => {
    ...
    const { env: envEmpty } = buildCleanEnv({
      processEnv: baseProcessEnv,
      dotenvVars: { YOS_TMUX_PATH_PREPEND: '', YOS_TMUX_PATH_APPEND: '' },
      platform: 'linux',
    });
  ...
  it('PREPEND dedupes with system paths preserving first occurrence', () => {
    const dotenvVars = { YOS_TMUX_PATH_PREPEND: '/usr/bin' };
    const { env } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
    const parts = env.PATH.split(':');
  ...
  it('manifest + .env keys merge and dedupe', () => {
    ...
    const dotenvVars = { YOS_TMUX_ENV: 'B,C', YOS_TMUX_INHERIT: 'X,Y', A: '1', B: '2', C: '3' };
  ...
  it('works without manifest (backward compat)', () => {
    const dotenvVars = { YOS_TMUX_ENV: 'TZ', TZ: 'Asia/Shanghai' };
    const { env } = buildCleanEnv({ processEnv: baseProcessEnv, dotenvVars, platform: 'linux' });
...
describe('buildCompatEnv', () => {
  ...
  it('overrides with YOS_TMUX_ENV manifest vars', () => {
    const processEnv = { MY_VAR: 'old_value' };
    const dotenvVars = { YOS_TMUX_ENV: 'MY_VAR', MY_VAR: 'new_value' };
    const { env } = buildCompatEnv({ processEnv, dotenvVars });
  ...
  it('does not inject GH_PROMPT_DISABLED, Homebrew paths, or PATH manifest', () => {
    ...
    const dotenvVars = {
      YOS_TMUX_PATH_PREPEND: '/custom/pre',
      YOS_TMUX_PATH_APPEND: '/custom/post',
    };
    const { env } = buildCompatEnv({ processEnv, dotenvVars });
...
describe('deployManifestTemplate', () => {
  it('creates manifest from template when missing', () => {
    ...
    const status = deployManifestTemplate(templatePath, yosDir);
    assert.equal(status, 'created');
    const dest = path.join(yosDir, '.yos', 'runtime-env.manifest');
  ...
  it('does not overwrite existing manifest', () => {
    ...
    const yosDir = path.join(tmpDir, 'yos');
    const yosSubdir = path.join(yosDir, '.yos');
    fs.mkdirSync(yosSubdir, { recursive: true });
    fs.writeFileSync(path.join(yosSubdir, 'runtime-env.manifest'), 'env USER_CUSTOM\n');

    const status = deployManifestTemplate(templatePath, yosDir);
    assert.equal(status, 'exists');
    assert.equal(
      fs.readFileSync(path.join(yosSubdir, 'runtime-env.manifest'), 'utf8'),
      'env USER_CUSTOM\n',
    );
  });
