...
describe('renderCodexProjectConfig', () => {
  ...
  it('preserves unknown top-level keys, sections, and feature flags while updating yos keys', () => {
    const existing = [
      '# User comment',
      'user_added = "keep"',
    ...
    assert.match(content, /^# YOS project-level Codex config\./);
  ...
  it('replaces yos-owned notice sections exactly without touching dotted siblings', () => {
    const existing = [
      '[notice]',
...
describe('renderCodexGlobalConfig', () => {
  it('includes trust declaration for the project directory', () => {
    const content = renderCodexGlobalConfig('/home/user/yos');
    assert.match(content, /\[projects\."\/home\/user\/yos"\]\ntrust_level = "trusted"/);
  });
  ...
  it('preserves unrelated trust entries', () => {
    ...
    const content = renderCodexGlobalConfig('/home/user/yos', existing);
  ...
  it('includes openai_base_url when provided', () => {
    const content = renderCodexGlobalConfig('/home/user/yos', '', { openaiBaseUrl: 'https://proxy.example.com/v1' });
  ...
  it('preserves existing openai_base_url when yos has no value', () => {
  ...
  it('overwrites existing openai_base_url when yos has a value', () => {
...
describe('writeCodexConfig', () => {
  it('writes project-level config and global config to separate locations', () => {
    ...
    const projectDir = path.join(fakeYOSDir, 'workspace', 'project-a');
  ...
  it('preserves unrelated trusted projects in global config', () => {
    const globalConfigPath = path.join(fakeHome, '.codex', 'config.toml');
    const projectDir = path.join(fakeYOSDir, 'workspace', 'project-b');
  ...
  it('preserves existing project-level config while regenerating yos keys', () => {
    const projectDir = path.join(fakeYOSDir, 'workspace', 'project-c');
    const projectConfigPath = path.join(path.resolve(projectDir), '.codex', 'config.toml');
