...
function writeSpec(spec) {
  const specPath = path.join(os.tmpdir(), `.yos-test-spec-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
...
describe('tmux-launcher', () => {
  ...
  it('deletes spec file before spawn', () => {
    ...
    const specPath = path.join(os.tmpdir(), `.yos-test-spec-delete-${Date.now()}.json`);
    const checkScript = `
      const fs = require('fs');
  ...
  it('writes exit log when exitLogFile is set', () => {
    const exitLog = path.join(os.tmpdir(), `.yos-test-exit-log-${Date.now()}`);
