...
function makeTmpDir() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'yos-fs-utils-test-'));
  tmpDirs.push(tmpDir);
  return tmpDir;
}
...
describe('syncTree', () => {
  ...
  it('restores from a nested backup while preserving excluded directories', () => {
    ...
    fs.mkdirSync(path.join(destDir, '.yos'), { recursive: true });
    ...
    fs.writeFileSync(path.join(destDir, '.yos', 'manifest.json'), '{}', 'utf8');

    syncTree(srcDir, destDir, { excludes: ['node_modules', '.backup', '.yos'] });
    ...
    assert.equal(fs.readFileSync(path.join(destDir, '.yos', 'manifest.json'), 'utf8'), '{}');
  });
});
