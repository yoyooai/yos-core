...
describe('step8_startService', () => {
  it('retries deleted services through ecosystem restart instead of pm2 start <name>', () => {
    ...
    assert.equal(result.status, 'done');
    assert.equal(calls.some((call) => call.type === 'ecosystem' && call.names[0] === 'yos-demo'), true);
    assert.equal(calls.some((call) => call.type === 'exec' && call.cmd === 'pm2 start yos-demo 2>/dev/null'), false);
...
describe('component upgrade rollback', () => {
  it('restores from skillDir/.backup/<timestamp> and preserves backup metadata', () => {
    ...
    fs.mkdirSync(path.join(skillDir, '.yos'), { recursive: true });
    ...
    fs.writeFileSync(path.join(skillDir, '.yos', 'manifest.json'), '{}\n', 'utf8');
    fs.writeFileSync(path.join(skillDir, '.yos', 'other-metadata.json'), 'keep\n', 'utf8');
    ...
    // #715 commit-boundary contract: rollback never touches .yos because
    ...
    assert.equal(fs.readFileSync(path.join(skillDir, '.yos', 'manifest.json'), 'utf8'), '{}\n');
    assert.equal(fs.readFileSync(path.join(skillDir, '.yos', 'other-metadata.json'), 'utf8'), 'keep\n');
...
describe('step11_startCoreServices', () => {
  ...
  it('uses the module default restart helper when no restart dep is injected', () => {
    ...
    fs.writeFileSync(ecosystemPath, 'module.exports = { apps: [] };\n', 'utf8');
    fs.writeFileSync(pm2Path, `#!/bin/sh\necho "$@" >> "${logPath}"\nif [ "$1" = "jlist" ]; then echo '[{"name":"activity-monitor","pm_id":3,"pm2_env":{"status":"online","YOS_PACKAGE_ROOT":"${tmpDir}"}}]'; fi\n`, { mode: 0o755 });
  ...
  it('fails before saving when pm2 jlist lacks activity-monitor package-root env', () => {
    ...
    try {
      ...
      assert.match(result.error, /YOS_PACKAGE_ROOT/);
      assert.match(fs.readFileSync(logPath, 'utf8'), /^jlist$/m);
