...
function makeEnv() {
  ...
  fs.mkdirSync(path.join(yosDir, '.claude', 'skills', 'activity-monitor', 'scripts'), { recursive: true });
  fs.writeFileSync(path.join(yosDir, '.claude', 'skills', 'activity-monitor', 'scripts', 'session-start-orchestrator.js'), '// stub');
  return { root, homeDir, yosDir };
}
...
describe('Codex SessionStart boundary', () => {
  it('never installs the split assembler as a SessionStart command', () => {
    const { yosDir } = makeEnv();
    const commands = coreSessionStartCommands(yosDir);
    assert.ok(commands.length > 0);
    assert.equal(commands.some(command => command.includes('assembler.mjs')), false);
    assert.equal(commands.some(command => command.includes('.yos/instructions')), false);
  });
});

function writeTrustedState({ homeDir, yosDir, hash = 'sha256:core' }) {
  const key = hookKeyFor({ yosDir, event: 'SessionStart', groupIndex: 0, hookIndex: 0 });
...
describe('Codex core hook installer', () => {
  it('upserts the core SessionStart hook and preserves other groups', () => {
    const { yosDir } = makeEnv();
    const hooksPath = codexHooksPath(yosDir);
    ...
    const first = installCoreCodexHook({ yosDir });
    const second = installCoreCodexHook({ yosDir });
  ...
  it('migrates old flat-array hooks.json and uninstall removes only core hook', () => {
    ...
    fs.writeFileSync(hooksPath, JSON.stringify([
      ...
      { event: 'SessionStart', command: `node ${path.join(yosDir, '.claude', 'skills', 'activity-monitor', 'scripts', 'session-start-orchestrator.js')}`, timeout: 10 },
    ], null, 2) + '\n');

    const installed = installCoreCodexHook({ yosDir });
    ...
    const removed = uninstallCoreCodexHook({ yosDir });
  ...
  it('uninstall removes only the core hook from a mixed hook group', () => {
    const { yosDir } = makeEnv();
    const hooksPath = codexHooksPath(yosDir);
    const coreCommand = `node ${path.join(yosDir, '.claude', 'skills', 'activity-monitor', 'scripts', 'session-start-orchestrator.js')}`;
    fs.mkdirSync(path.dirname(hooksPath), { recursive: true });
    fs.writeFileSync(hooksPath, JSON.stringify({
      hooks: {
        SessionStart: [{
...
describe('Codex hook trust backstop', () => {
  it('re-trusts all hooks, writes marker, then skips app-server in steady state', () => {
    const { homeDir, yosDir } = makeEnv();
    installCoreCodexHook({ yosDir });

    let spawnCalls = 0;
    const spawnSyncImpl = () => {
      spawnCalls++;
      writeTrustedState({ homeDir, yosDir });
      return { status: 0, stdout: JSON.stringify({ ok: true, trusted: 1 }) + '\n', stderr: '' };
    };
    const execFileSyncImpl = () => 'codex-cli 0.142.2\n';

    const first = ensureCodexHooksTrusted({ yosDir, projectDir: yosDir, homeDir, execFileSyncImpl, spawnSyncImpl });
    const second = ensureCodexHooksTrusted({ yosDir, projectDir: yosDir, homeDir, execFileSyncImpl, spawnSyncImpl });

    assert.equal(first.trusted, true);
    assert.equal(first.reason, 'project_hooks_feature_off');
    assert.equal(second.skipped, true);
    assert.equal(spawnCalls, 1);
    assert.ok(fs.existsSync(codexTrustMarkerPath(yosDir)));
  });
  ...
  it('detects unchanged hooks.json with corrupted trusted_hash value', () => {
    ...
    const spawnSyncImpl = () => {
      spawnCalls++;
      writeTrustedState({ homeDir, yosDir, hash: 'sha256:good' });
      return { status: 0, stdout: JSON.stringify({ ok: true, trusted: 1 }) + '\n', stderr: '' };
    };
    const execFileSyncImpl = () => 'codex-cli 0.142.2\n';

    ensureCodexHooksTrusted({ yosDir, projectDir: yosDir, homeDir, execFileSyncImpl, spawnSyncImpl });
    writeTrustedState({ homeDir, yosDir, hash: 'sha256:bad' });
  ...
  it('re-trusts after a hook group index shift changes hooks.state keys', () => {
    ...
    const spawnSyncImpl = () => {
      spawnCalls++;
      const key = hookKeyFor({ yosDir, event: 'SessionStart', groupIndex: 1, hookIndex: 0 });
      const globalConfigPath = codexGlobalConfigPath(homeDir);
      fs.mkdirSync(path.dirname(globalConfigPath), { recursive: true });
      fs.writeFileSync(globalConfigPath, [
        '[features]',
  ...
  it('detects hooks feature disabled and restores it before trusting', () => {
    ...
    fs.writeFileSync(codexProjectConfigPath(yosDir), '[features]\nhooks = false\n');
    const result = ensureCodexHooksTrusted(args);

    assert.equal(result.trusted, true);
    assert.equal(result.reason, 'project_hooks_feature_off');
    assert.equal(spawnCalls, 2);
    assert.match(fs.readFileSync(codexProjectConfigPath(yosDir), 'utf8'), /hooks = true/);
  });

  it('detects Codex version change', () => {
    const { homeDir, yosDir } = makeEnv();
    installCoreCodexHook({ yosDir });
    const key = writeTrustedState({ homeDir, yosDir });
    const hooksContent = fs.readFileSync(codexHooksPath(yosDir), 'utf8');
    fs.mkdirSync(path.dirname(codexProjectConfigPath(yosDir)), { recursive: true });
    fs.writeFileSync(codexProjectConfigPath(yosDir), '[features]\nhooks = true\n');
    fs.writeFileSync(codexTrustMarkerPath(yosDir), JSON.stringify({
      hooksHash: crypto.createHash('sha256').update(hooksContent).digest('hex'),
      codexVersion: 'codex-cli 0.142.1',
      trustSnapshot: extractTrustSnapshot(readHooksState(codexGlobalConfigPath(homeDir)), codexHooksPath(yosDir)),
      key,
    }, null, 2));
