...
describe('PM2 ecosystem restart helpers', () => {
  ...
  it('uses ecosystem restart for managed processes when config exists', () => {
    ...
    restartManagedProcess('yos-wecom', {
    ...
    assert.deepStrictEqual(calls, [
      {
        cmd: 'pm2 start "/tmp/component-ecosystem.config.cjs" --only "yos-wecom" --update-env 2>/dev/null',
        opts: { stdio: 'pipe' },
      },
    ]);
  });
  ...
  it('falls back to plain restart only when no ecosystem config exists', () => {
    ...
    restartManagedProcess('yos-custom', {
    ...
    assert.deepStrictEqual(calls, [
      {
        cmd: 'pm2 restart "yos-custom" 2>/dev/null',
        opts: { stdio: 'pipe' },
      },
      {
        cmd: 'pm2 save 2>/dev/null',
