...
describe('desiredClaudeHooks', () => {
  it('omits assembler hooks until the assembler is materialized', () => {
    ...
    fs.rmSync(yosDir, { recursive: true, force: true });
  });
  ...
  it('real postinstall does not publish dead assembler hooks before init', () => {
    ...
    execFileSync(process.execPath, [POSTINSTALL_PATH], {
      ...
      env: {
        ...process.env,
        CI: '',
        HOME: path.dirname(yosDir),
        YOS_DIR: yosDir,
        YOS_SKIP_POSTINSTALL: '1',
      },
    });
  ...
  it('the real clear hook rebuilds changed user instructions in a sandbox', () => {
    ...
    const clearGroup = desiredClaudeHooks({ yosDir }).SessionStart.find(group => group.matcher === 'clear');
  ...
  it('uses absolute per-shard SessionStart orchestrator commands for all matchers', () => {
    ...
    assert.equal(groups.length, 3);
    for (const group of groups) {
      assert.match(group.hooks[0].command, /\.yos\/instructions\/assembler\.mjs/);
...
describe('persistInstalledSettingsAndSyncCoupledThreshold', () => {
  ...
  it('writes settings before syncing the paired threshold', () => {
    ...
    const result = persistInstalledSettingsAndSyncCoupledThreshold({
      ...
      settingsPath: '/tmp/yos/.claude/settings.json',
      modelBackfilled: true,
    ...
    assert.deepEqual(calls, [
      ['mkdir', '/tmp/yos/.claude', { recursive: true }],
      ['writeSettings', '/tmp/yos/.claude/settings.json', { model: 'opus[1m]' }],
      ['syncThreshold', true],
    ]);
  });
  ...
  it('backs up existing settings before writing', () => {
    ...
    assert.deepEqual(calls, [
      ...
      ['writeSettings', '/tmp/yos/.claude/settings.json', { hooks: {} }],
    ]);
  });
  ...
  it('prunes old .bak backups after a successful write, keeping the newest N', () => {
    ...
    assert.deepEqual(unlinked.sort(), [
      '/tmp/yos/.claude/settings.json.bak.100',
      '/tmp/yos/.claude/settings.json.bak.200',
    ]);
  });
...
describe('syncCodexConfig', () => {
  it('refreshes stale codex config when codex state exists outside codex runtime', () => {
    ...
    assert.equal(result.changed, true);
    assert.deepEqual(writes, ['/tmp/yos']);
  ...
  it('detects real drift in yos-managed project keys', () => {
    const homeDir = '/tmp/home';
    const projectDir = '/tmp/yos';
    const globalConfigPath = path.join(homeDir, '.codex', 'config.toml');
    const projectConfigPath = path.join(projectDir, '.codex', 'config.toml');
...
function makeStandardOldSessionStartGroup(matcher) {
  return {
    matcher,
    hooks: [
      makeHook(yosHookPath('skills/yos-memory/scripts/session-start-inject.js'), 10000),
      makeHook(yosHookPath('skills/comm-bridge/scripts/c4-session-init.js'), 10000),
      makeHook(yosHookPath('skills/activity-monitor/scripts/session-foreground.js'), 5000),
      makeHook(yosHookPath('skills/activity-monitor/scripts/session-start-prompt.js'), 5000),
    ],
  };
}
...
describe('extractScriptPath', () => {
  ...
  it('uses the last shell segment so source prefixes do not win', () => {
    assert.equal(
      extractScriptPath('source ~/.nvm/nvm.sh && node ~/yos/.claude/skills/a/scripts/b.js'),
      path.join(os.homedir(), 'yos/.claude/skills/a/scripts/b.js')
    );
  });

  it('skips interpreter flags before the script argument', () => {
    assert.equal(
      extractScriptPath('node --trace-warnings ~/yos/.claude/skills/a/scripts/b.js --mode startup'),
      path.join(os.homedir(), 'yos/.claude/skills/a/scripts/b.js')
    );
  });

  it('keeps quoted paths with spaces as one script argument', () => {
    const scriptPath = yosHookPath('skills/a dir/scripts/b.js');
    assert.equal(
...
describe('hookScriptKey', () => {
  it('normalizes absolute, home-relative, and backslash yos-root script paths to the same registry key', () => {
    assert.equal(
      hookScriptKey('node ~/yos/.claude/skills/activity-monitor/scripts/session-start-orchestrator.js'),
      'skills/activity-monitor/scripts/session-start-orchestrator.js'
    );
    assert.equal(
      hookScriptKey(`node ${yosHookPath('skills/activity-monitor/scripts/session-start-orchestrator.js')}`),
      'skills/activity-monitor/scripts/session-start-orchestrator.js'
    );
    assert.equal(
      hookScriptKey(`node ${yosHookPath('skills/activity-monitor/scripts/session-start-orchestrator.js').replaceAll('/', '\\')}`),
      'skills/activity-monitor/scripts/session-start-orchestrator.js'
    );
  });

  it('keeps paths outside the yos .claude root as full normalized paths even when suffixes collide', () => {
    assert.equal(
    ...
    assert.equal(
      hookScriptKey('node /opt/custom/skills/yos-memory/scripts/session-start-inject.js'),
      '/opt/custom/skills/yos-memory/scripts/session-start-inject.js'
    );
    assert.equal(
  ...
  it('resolves relative YOS_DIR before checking the yos .claude root', () => {
    const previousYOSDir = process.env.YOS_DIR;
    process.env.YOS_DIR = 'relative-yos';
    try {
      const absoluteCoreHook = path.resolve(
        'relative-yos/.claude/skills/activity-monitor/scripts/session-start-orchestrator.js'
      );
      assert.equal(
      ...
      if (previousYOSDir === undefined) {
        delete process.env.YOS_DIR;
      } else {
        process.env.YOS_DIR = previousYOSDir;
      }
    }
  });
});
...
describe('core hook registry', () => {
  it('recognizes the materialized assembler under an injected yos root', () => {
    const yosDir = path.join(os.tmpdir(), 'isolated-yos-root');
    const assembler = path.join(yosDir, '.yos', 'instructions', 'assembler.mjs');
    assert.equal(
      isCoreManaged({ type: 'command', command: `node ${assembler}` }, { yosDir }),
      true,
    );
  });
...
describe('syncHooks SessionStart orchestrator convergence', () => {
        ...
        SessionStart: [
          {
            matcher: 'startup',
            hooks: [
              makeHook(yosHookPath('skills/yos-memory/scripts/session-start-inject.js'), 9000),
              makeHook(yosHookPath('skills/comm-bridge/scripts/c4-session-init.js'), 10000),
              makeHook(yosHookPath('skills/activity-monitor/scripts/session-start-prompt.js'), 5000),
            ],
          },
  ...
  it('does not claim user hooks outside the yos .claude root whose suffix collides with current or retired registry keys', () => {
    const installed = {
      hooks: {
        SessionStart: [
          {
            ...
            hooks: [
              ...
              makeHook('/opt/custom/skills/yos-memory/scripts/session-start-inject.js', 7000),
    ...
    assert.ok(startup.hooks.some(h =>
      h.command === 'node /opt/custom/skills/yos-memory/scripts/session-start-inject.js' &&
      h.timeout === 7000
    ));
    assert.ok(startup.hooks.some(h =>
...
describe('component shard claim boundary (opt-in contract)', () => {
  ...
  it('claims a declared legacy hook: old entry removed, --shard command generated, chain covers it', async () => {
    const yosDir = makeDeclaredYOSDir();
        ...
        SessionStart: [
          {
            ...
            hooks: [
              ...
              makeHook(yosHookPath('skills/role-manager/role-inject-hook.sh'), 10000),
              ...
              makeHook(yosHookPath('skills/other-component/scripts/other-hook.js'), 5000),
            ],
          },
        ],
      },
    ...
    const result = syncHooks(installed, {}, {
      log: noopLog,
      desiredHooks: desired({ yosDir }),
      claimedKeys: claimedHookBaseKeys({ yosDir }),
    });
  ...
  it('rejects absolute claim paths at declaration time so user hooks can never be claimed', async () => {
    ...
    const claimed = claimedHookBaseKeys({ yosDir });
...
describe('syncHooks forward pass', () => {
      ...
      hooks: {
        SessionStart: [
          { matcher: 'startup', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/comm-bridge/scripts/c4-session-init.js', timeout: 10000 },
            { type: 'command', command: 'node ~/yos/.claude/skills/yos-memory/scripts/session-start-inject.js', timeout: 10000 },
      ...
      hooks: {
        SessionStart: [
          makeMatcherGroup('startup', [
            '~/yos/.claude/skills/comm-bridge/scripts/c4-session-init.js',
            '~/yos/.claude/skills/yos-memory/scripts/session-start-inject.js',
            '~/yos/.claude/skills/activity-monitor/scripts/session-start-prompt.js',
          ]),
  ...
  it('registers ToolWatchdog activity hooks during upgrade when event matcher groups already exist', () => {
    ...
    const template = {
      hooks: {
        PreToolUse: [
          { matcher: '', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/activity-monitor/scripts/hook-activity.js', async: true, timeout: 5 },
          ]},
        ],
        PostToolUse: [
          { matcher: '', hooks: [
  ...
  it('updates command/timeout drift in existing matcher group', () => {
    const installed = {
      hooks: {
        SessionStart: [
          { matcher: 'startup', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/a/scripts/a.js', timeout: 5000 },
          ]},
        ],
      },
    };
    const template = {
      hooks: {
        SessionStart: [
          { matcher: 'startup', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/a/scripts/a.js', timeout: 10000 },
          ]},
        ],
      },
    };
  ...
  it('reverse pass removes obsolete core hooks', () => {
    const installed = {
      hooks: {
        SessionStart: [
          { matcher: 'startup', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/old-skill/scripts/old.js', timeout: 5000 },
          ]},
        ],
      },
    };
  ...
  it('reverse pass preserves user hooks not in template', () => {
    ...
    const template = {
      hooks: {
        SessionStart: [
          makeMatcherGroup('startup', ['~/yos/.claude/skills/a/scripts/a.js']),
        ],
      },
    };
  ...
  it('reverse pass removes core hook from one matcher when template removes it (matcher-aware)', () => {
    ...
    const installed = {
      hooks: {
        SessionStart: [
          { matcher: 'startup', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/activity-monitor/scripts/session-start-prompt.js', timeout: 5000 },
          ]},
          { matcher: 'clear', hooks: [
            { type: 'command', command: 'node ~/yos/.claude/skills/activity-monitor/scripts/session-start-prompt.js', timeout: 5000 },
          ]},
    ...
    const template = {
      hooks: {
        SessionStart: [
          makeMatcherGroup('startup', ['~/yos/.claude/skills/activity-monitor/scripts/session-start-prompt.js']),
