...
beforeEach(() => {
  ...
  process.env.YOS_GH_RETRY_DELAY_MS = '10,10';
});
...
describe('withRateLimitRetrySync', () => {
  ...
  it('disables retries when YOS_GH_RETRY_DELAY_MS is empty', () => {
    process.env.YOS_GH_RETRY_DELAY_MS = '';
    let calls = 0;
    assert.throws(
...
`;
  ...
  it('recovers from no-token rate limiting via the outer retry loop only (#705)', async () => {
    delete process.env.GITHUB_TOKEN;
    delete process.env.GH_TOKEN;
    process.env.YOS_GH_RETRY_DELAY_MS = '0';
  ...
  it('makes three public calls when all three no-token rate-limit attempts fail (#705)', async () => {
    delete process.env.GITHUB_TOKEN;
    delete process.env.GH_TOKEN;
    process.env.YOS_GH_RETRY_DELAY_MS = '0,0';
