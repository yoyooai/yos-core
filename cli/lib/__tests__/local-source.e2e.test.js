...
function runAdd({ cwd, yosDir, target, env = {} }) {
  const result = spawnSync(process.execPath, [CLI, 'add', target, '--json'], {
    cwd,
    env: { ...process.env, YOS_DIR: yosDir, ...env },
    encoding: 'utf8',
    timeout: 30000,
  });
...
function runCli({ cwd, yosDir, args, env = {} }) {
...
function readInstalled(yosDir, name) {
...
describe('yos add local source E2E', () => {
  it('installs an explicit relative directory through the complete add pipeline', () => {
    ...
    const output = runAdd({ cwd: root, yosDir, target: `./${sourceName}` });
    const { components, skillDir } = readInstalled(yosDir, 'local-directory-e2e');
    ...
    assert.equal(fs.existsSync(path.join(skillDir, '.yos', 'manifest.json')), true);
  });
  ...
  it('uses an absolute local path in --check confirmation output', () => {
    ...
    const result = runCli({ cwd: root, yosDir, args: ['add', `./${sourceName}`, '--check', '--json'] });
  ...
  it('does not let a cwd directory hijack a bare component name', () => {
    ...
    const result = runCli({ cwd: root, yosDir, args: ['add', 'not-in-registry', '--json'] });

    assert.equal(result.status, 1);
    const output = JSON.parse(result.stdout);
    assert.equal(output.error, 'not_found');
    assert.equal(fs.existsSync(path.join(yosDir, '.claude', 'skills', 'hijacked-local')), false);
  });
  ...
  it('installs a local tarball and records its VERSION metadata', () => {
    ...
    const output = runAdd({ cwd: root, yosDir, target: './component.tar.gz' });
    const { components, skillDir } = readInstalled(yosDir, 'local-tarball-e2e');
...
`, { mode: 0o755 });
    ...
    const output = runAdd({
      cwd: root,
      yosDir,
      target: 'example/yos-github-fixture@1.0.0',
      env: {
    ...
    const { components, skillDir } = readInstalled(yosDir, 'github-fixture');

    assert.equal(output.success, true);
    assert.equal(output.version, '1.0.0');
    assert.equal(components['github-fixture'].repo, 'example/yos-github-fixture');
    assert.deepEqual(components['github-fixture'].source, {
      type: 'github-release',
      repo: 'example/yos-github-fixture',
      ref: '1.0.0',
      refType: 'tag',
    });
  ...
  it('rejects upgrade checks for locally installed components with reinstall guidance', () => {
    ...
    runAdd({ cwd: root, yosDir, target: './local-upgrade-source' });
  ...
  it('fails the upgrade --all JSON aggregate for a local-only install', () => {
    ...
    runAdd({ cwd: root, yosDir, target: './local-upgrade-all-source' });
    ...
    assert.equal(
      fs.readFileSync(
        path.join(yosDir, '.claude', 'skills', 'local-upgrade-all-e2e', 'payload.txt'),
        'utf8'
      ),
  ...
  it('fails the upgrade --all JSON aggregate while preserving remote updates', () => {
    ...
    runAdd({ cwd: root, yosDir, target: './local-mixed-source' });

    const remoteName = 'remote-mixed-e2e';
    writeSkill(
      path.join(yosDir, '.claude', 'skills', remoteName),
    ...
    components[remoteName] = {
      version: '1.0.0',
      repo: 'example/yos-remote-mixed-e2e',
      source: {
        type: 'github-release',
        repo: 'example/yos-remote-mixed-e2e',
        ref: '1.0.0',
