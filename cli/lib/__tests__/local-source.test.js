...
function makeTmpDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'yos-local-source-test-'));
  tmpDirs.push(dir);
  return dir;
}
...
describe('local component source resolution', () => {
  ...
  it('recognizes only explicit local path forms without cwd-based bare-name collisions', () => {
    ...
    for (const value of [
      ...
      'lark',
      'example/yos-demo',
      'https://github.com/example/yos-demo/archive/refs/tags/v1.0.0.tar.gz',
    ]) {
  ...
  it('excludes generated and private runtime directories when copying a local source', () => {
    ...
    for (const excluded of ['node_modules', '.yos', '.backup', '.git']) {
  ...
  it('keeps an absent org/repo target on the GitHub path', async () => {
    const resolved = await resolveTarget('example/yos-demo@9.8.7');

    assert.equal(resolved.name, 'demo');
    assert.equal(resolved.repo, 'example/yos-demo');
    assert.deepEqual(resolved.source, {
      type: 'github-release',
      repo: 'example/yos-demo',
      ref: '9.8.7',
      refType: 'tag',
    });
  });
