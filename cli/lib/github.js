 ...
 * YOS_GH_RETRY_DELAY_MS (comma-separated, e.g. "1000,5000";
 * empty string disables retries) — used by tests and tunable in ops.
 */
function retryDelaysMs() {
  const env = process.env.YOS_GH_RETRY_DELAY_MS;
...
function notifyRetry(label, attempt, total, delayMs) {
  // stderr on purpose: `yos upgrade --json` reserves stdout for JSON
  console.error(
