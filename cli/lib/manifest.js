 ...
 */
function collectFiles(dir, baseDir, exclude = ['.git', '.yos', 'node_modules', '.backup']) {
  const files = [];
...
/**
 * Save manifest to the component's .yos/ directory.
 *
...
/**
 * Load manifest from a component's .yos/ directory.
 *
 * @param {string} dir - Component root directory
 ...
 * Stored in componentDir/.yos/originals/ mirroring the source structure.
 *
...
export function recoverMergeBaseline(componentDir) {
  const yosDir = path.join(componentDir, MANIFEST_DIR);
  if (!fs.existsSync(yosDir)) return;

  const originalsDir = path.join(yosDir, ORIGINALS_DIR);
  const stagedManifest = path.join(yosDir, MANIFEST_TMP);
  const stagedOriginals = path.join(yosDir, ORIGINALS_NEW);
  const legacyBackup = path.join(yosDir, ORIGINALS_LEGACY_BAK);
  ...
  if (fs.existsSync(legacyBackup)) {
    ...
    if (dirMatchesManifest(originalsDir, manifest)) {
      ...
      throw new Error(
        `baseline recovery: ${ORIGINALS_LEGACY_BAK} present but neither active originals nor the backup match the manifest — manual inspection required in ${yosDir}`
      );
    }
  }
}
 ...
 * .yos (issue #715 review findings, R1-R3).
 *
 * @param {string} componentDir - Installed component root directory
...
export function saveMergeBaseline(componentDir, sourceDir, manifest) {
  ...
  const manifestPath = path.join(yosDir, MANIFEST_FILE);
  const originalsDir = path.join(yosDir, ORIGINALS_DIR);
  const stagedManifest = path.join(yosDir, MANIFEST_TMP);
  const stagedOriginals = path.join(yosDir, ORIGINALS_NEW);
  ...
  // roll business files back while the manifest stayed advanced (.yos is
