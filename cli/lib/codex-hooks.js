...
export function codexHooksPath(yosDir) {
  return path.join(path.resolve(yosDir), '.codex', 'hooks.json');
}

export function codexTrustMarkerPath(yosDir) {
  return path.join(path.resolve(yosDir), '.codex', 'hooks.trust.json');
}
...
export function coreSessionStartCommand(yosDir) {
 ...
 */
export function coreSessionStartCommands(yosDir) {
  const base = coreSessionStartCommand(yosDir);
...
export function isCoreCodexHook(command, yosDir) {
...
export function installCoreCodexHook({ yosDir }) {
  const filePath = codexHooksPath(yosDir);
  ...
  const preserved = existing
    .map((group) => {
      ...
      return { ...group, hooks: group.hooks.filter(h => !isCoreCodexHook(h?.command, yosDir)) };
    })
...
export function uninstallCoreCodexHook({ yosDir }) {
  const filePath = codexHooksPath(yosDir);
  const config = readCodexHooksConfig(filePath);
  ...
  for (const event of Object.keys(config.hooks)) {
    ...
    config.hooks[event] = config.hooks[event]
      .map((group) => {
        ...
        const hooks = group.hooks.filter(h => !isCoreCodexHook(h?.command, yosDir));
...
export function isCodexTrustValid({
  ...
  const markerPath = codexTrustMarkerPath(yosDir);
...
export function hookKeyFor({ yosDir, event, groupIndex, hookIndex }) {
  return `${codexHooksPath(yosDir)}:${eventKey(event)}:${groupIndex}:${hookIndex}`;
}
...
const cwd = process.env.YOS_CODEX_TRUST_CWD;
const codexBin = process.env.YOS_CODEX_BIN || 'codex';
...
send('initialize', {
  clientInfo: {
    name: 'yos_core_codex_hook_trust',
    title: 'YOS Core Codex Hook Trust',
    version: '0.1.0'
  },
...
`;
  ...
  const result = spawnSyncImpl(process.execPath, ['-e', helper], {
    ...
    env: {
      ...process.env,
      YOS_CODEX_TRUST_CWD: yosDir,
      YOS_CODEX_BIN: codexBin,
    },
    encoding: 'utf8',
    timeout: timeoutMs,
  });
...
export function ensureCodexHooksTrusted({
  ...
  const validity = isCodexTrustValid({ yosDir, projectDir, homeDir, codexVersion });
  if (validity.valid) return { trusted: false, skipped: true, reason: validity.reason };

  ensureHooksFeature({ projectDir, homeDir });
  const trust = trustCodexHooksWithAppServer({ yosDir, codexBin, spawnSyncImpl });
  if (!trust.ok) {
  ...
  writeTrustMarker(codexTrustMarkerPath(yosDir), {
