...
function step1_stopService(ctx) {
  ...
  const serviceName = parsed?.frontmatter?.lifecycle?.service?.name || `yos-${ctx.component}`;
...
function step2_backup(ctx) {
  ...
  try {
    copyTree(ctx.skillDir, backupDir, { excludes: ['node_modules', '.backup', '.yos'] });
...
export function rollback(ctx, deps = {}) {
  ...
  if (ctx.backupDir && fs.existsSync(ctx.backupDir)) {
    try {
      syncTree(ctx.backupDir, ctx.skillDir, { excludes: ['node_modules', '.backup', '.yos'] });
