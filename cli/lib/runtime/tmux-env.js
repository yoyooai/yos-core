 ...
 */
export function deployManifestTemplate(templatePath, yosDir) {
...
/**
 * Load and parse runtime-env.manifest from YOS_DIR/.yos/.
 * Returns empty manifest if file is missing.
 */
export function loadRuntimeEnvManifest(yosDir, warnings = []) {
  try {
    const content = fs.readFileSync(
      path.join(yosDir, '.yos', 'runtime-env.manifest'), 'utf8',
    );
...
/**
 * Build a clean environment object (default, or YOS_CLEAN_ENV!=false).
 * Merges manifest file directives with legacy .env keys (YOS_TMUX_ENV, etc.).
...
export function buildCleanEnv({ processEnv, dotenvVars, manifest, platform, uid, execPath }) {
  ...
  const dotenvPrepend = parsePathManifest(
    dotenvVars.YOS_TMUX_PATH_PREPEND, warnings, 'YOS_TMUX_PATH_PREPEND',
  );
  const allPrepend = [...new Set([...m.pathPrepend, ...dotenvPrepend])];

  const dotenvAppend = parsePathManifest(
    dotenvVars.YOS_TMUX_PATH_APPEND, warnings, 'YOS_TMUX_PATH_APPEND',
  );
  ...
  const dotenvEnvNames = parseManifest(dotenvVars.YOS_TMUX_ENV, warnings);
  ...
  const dotenvInheritNames = parseManifest(dotenvVars.YOS_TMUX_INHERIT, warnings);
...
/**
 * Build a compat environment object (YOS_CLEAN_ENV=false).
...
export function buildCompatEnv({ processEnv, dotenvVars }) {
  ...
  // Override with YOS_TMUX_ENV manifest vars from dotenvVars
  const warnings = [];
  const tmuxEnvNames = parseManifest(dotenvVars.YOS_TMUX_ENV, warnings);
...
export function writeLaunchSpec(spec) {
  const specPath = path.join(os.tmpdir(), `.yos-launch-${process.pid}-${Date.now()}.json`);
