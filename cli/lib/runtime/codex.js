 ...
 * during `yos init` by writeCodexConfig().
 */
...
export class CodexAdapter extends RuntimeAdapter {
   ...
   *
   * We do NOT read ~/yos/.env — Codex CLI deliberately ignores OPENAI_API_KEY env vars,
  ...
  async launch(opts = {}) {
    ...
    const agentsDir = path.join(YOS_DIR, '.agents');
    ...
    if (tmuxHasSession(SESSION)) {
      ...
      const cmd = `cd "${YOS_DIR}"; ${codexCmd} "${kickPrompt}"; ${exitLogSnippet}`;
   ...
   */
  getHeartbeatDeps() {
    const pendingFile = path.join(YOS_DIR, 'activity-monitor', 'codex-heartbeat-pending.json');
