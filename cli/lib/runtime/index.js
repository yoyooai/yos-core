 ...
 *
 * The active runtime is stored in ~/.yos/config.json as { "runtime": "claude" }.
...
import { getYosConfig } from '../config.js';
...
export function getAdapter(name, config) {
  ...
  return new Cls(config ?? getYosConfig());
}

/**
 * Get an adapter instance for the currently configured runtime.
 * Reads 'runtime' from ~/.yos/config.json; defaults to 'claude'.
 *
 * @param {object} [config] - Optional config override
 * @returns {import('./base.js').RuntimeAdapter}
 */
export function getActiveAdapter(config) {
  const cfg = config ?? getYosConfig();
