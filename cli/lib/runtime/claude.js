...
const C4_CONTROL_PATH = path.join(YOS_DIR, '.claude', 'skills', 'comm-bridge', 'scripts', 'c4-control.js');
...
export class ClaudeAdapter extends RuntimeAdapter {
  ...
  async checkAuth() {
    ...
    let envBaseUrl = '';
    try {
      const envContent = fs.readFileSync(path.join(YOS_DIR, '.env'), 'utf8');
  ...
  async sendMessage(text) {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'yos-'));
    const tmpFile = path.join(tmpDir, 'msg.txt');
    const bufferName = `yos-${msgId}`;
  ...
  async launch(opts = {}) {
    ...
    _ensureOnboardingComplete(YOS_DIR);
    ...
    if (tmuxHasSession(SESSION)) {
      ...
      const cmd = `cd "${YOS_DIR}"; ${claudeCmd}; ${exitLogSnippet}`;
      await this.sendMessage(cmd);
    } else {
      // New session — launcher pipeline
      const dotenvVars = _readDotenvVars();
      const useCleanEnv = dotenvVars.YOS_CLEAN_ENV !== 'false';
      const manifest = useCleanEnv ? loadRuntimeEnvManifest(YOS_DIR) : undefined;
   ...
   */
  getHeartbeatDeps() {
    const pendingFile = path.join(YOS_DIR, 'activity-monitor', 'heartbeat-pending.json');
...
function _hasStartupHook() {
  try {
    const settingsPath = path.join(YOS_DIR, '.claude', 'settings.json');
