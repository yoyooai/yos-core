...
import { CADDYFILE, CADDY_BIN, getYosConfig } from './config.js';
 ...
 * @param {Array} httpRoutes - Array of { path, type, target, strip_prefix? }
 * @returns {string} Caddy configuration snippet with yos markers
 */
export function generateManualRouteSnippet(componentName, httpRoutes) {
  return [
    `    # BEGIN yos-component:${componentName}`,
    generateRouteBlocks(httpRoutes),
    `    # END yos-component:${componentName}`,
  ].join('\n');
}
...
export function applyCaddyRoutes(componentName, httpRoutes, deps = {}) {
  ...
  if (!isAvailable()) {
    return {
      ...
      message: 'YOS-managed Caddy is not available. HTTP routes were not configured automatically.',
    };
  }

  const beginMarker = `# BEGIN yos-component:${componentName}`;
  const endMarker = `# END yos-component:${componentName}`;
  ...
  // inserted into the first block, which is always the yos-managed one.
...
function validateAndDeploy(newContent, originalContent) {
  const tmpFile = `/tmp/Caddyfile.yos-${Date.now()}`;
