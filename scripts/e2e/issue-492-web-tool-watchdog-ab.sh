...
against a dedicated local YOS instance.

This script:
1. creates a detached worktree at <main-ref>
2. optionally runs skills/activity-monitor npm tests on main and current
3. deploys main into ~/yos, runs live normal/hang cases, captures evidence
4. deploys the current workspace into ~/yos, reruns the same cases
5. prints a summary and leaves ~/yos on the current workspace deploy
...
Options:
  ...
  --yos-dir <path>         Live YOS home (default: /home/ac/yos)
...
Assumptions:
  - ~/yos is a dedicated test environment
...
write_test_config() {
  YOS_DIR="$LIVE_YOS_DIR" TIMEOUT_SEC="$TIMEOUT_SEC" GRACE_SEC="$GRACE_SEC" COOLDOWN_SEC="$COOLDOWN_SEC" node --input-type=module <<'EOF'
import fs from 'node:fs';
import path from 'node:path';

const file = path.join(process.env.YOS_DIR, '.yos', 'config.json');
let data = {};
try {
...
wait_for_agent_online() {
  local timeout_sec="$1"
  local status_file="$LIVE_YOS_DIR/activity-monitor/agent-status.json"
  local start_sec="$SECONDS"
  while (( SECONDS - start_sec < timeout_sec )); do
    local state
...
save_snapshot() {
  ...
  save_new_lines "$LIVE_YOS_DIR/activity-monitor/tool-events.jsonl" "$events_line" "$out_dir/tool-events.new.jsonl"
  save_new_lines "$LIVE_YOS_DIR/activity-monitor/activity.log" "$activity_line" "$out_dir/activity.new.log"
  if [ -f "$LT_LOG" ]; then
  ...
  for file in \
    "$LIVE_YOS_DIR/activity-monitor/agent-status.json" \
    "$LIVE_YOS_DIR/activity-monitor/api-activity.json" \
    "$LIVE_YOS_DIR/activity-monitor/session-tool-state.json" \
    "$LIVE_YOS_DIR/activity-monitor/tool-watchdog-state.json" \
    "$LIVE_YOS_DIR/activity-monitor/tool-events.jsonl" \
    "$LIVE_YOS_DIR/activity-monitor/activity.log"; do
    if [ -f "$file" ]; then
...
run_main_normal_case() {
  ...
  events_line="$(line_count "$LIVE_YOS_DIR/activity-monitor/tool-events.jsonl")"
  activity_line="$(line_count "$LIVE_YOS_DIR/activity-monitor/activity.log")"
...
run_main_hang_case() {
  ...
  if sed -n "$((activity_line + 1)),\$p" "$LIVE_YOS_DIR/activity-monitor/activity.log" | grep -Fq 'Tool watchdog: sent Escape for WebFetch'; then
...
run_feat_hang_case() {
  ...
  wait_for_new_file_contains "$LIVE_YOS_DIR/activity-monitor/activity.log" "$activity_line" "Tool watchdog: sent Escape for WebFetch" "$observe_sec" || fail "feat hang: watchdog did not send Escape within ${observe_sec}s"
...
best_effort_restore_feature() {
  ...
  set +e
  log "best-effort restore: redeploying current workspace into ${LIVE_YOS_DIR}"
  mkdir -p "$LIVE_CLAUDE_DIR/skills/activity-monitor/scripts" "$LIVE_CLAUDE_DIR/skills/comm-bridge/scripts"
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --artifacts-dir)
      ...
      ;;
    --yos-dir)
      LIVE_YOS_DIR="${2:?missing value for --yos-dir}"
      shift 2
      ;;
    --session-name)
...
LIVE_CLAUDE_DIR="$LIVE_YOS_DIR/.claude"
ensure_artifacts_dir
write_metadata
ensure_main_worktree
