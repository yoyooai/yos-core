...
YOS_HOME="$HOME_DIR/yos"
BIN_DIR="$TMP_DIR/bin"
PM2_LOG="$TMP_DIR/pm2.log"

mkdir -p "$YOS_HOME/pm2" "$YOS_HOME/activity-monitor" "$YOS_HOME/.yos" "$HOME_DIR/.codex" "$BIN_DIR"

cat > "$YOS_HOME/pm2/ecosystem.config.cjs" <<'EOF'
module.exports = {
  apps: [
...
cat > "$YOS_HOME/.yos/config.json" <<'EOF'
{
  "runtime": "claude"
}
EOF
...
: > "$PM2_LOG"
node "$ROOT_DIR/cli/yos.js" restart >/dev/null
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only activity-monitor"
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only scheduler"
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only c4-dispatcher"
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only web-console"
...
echo "== runtime switch =="
: > "$PM2_LOG"
node "$ROOT_DIR/cli/yos.js" runtime codex >/dev/null
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only activity-monitor"
assert_log_contains "start $YOS_HOME/pm2/ecosystem.config.cjs --only c4-dispatcher"
assert_log_not_contains "restart activity-monitor"
assert_log_not_contains "restart c4-dispatcher"
...
: > "$PM2_LOG"
SKILL_DIR="$YOS_HOME/.claude/skills/demo"
mkdir -p "$SKILL_DIR"
...
module.exports = { apps: [{ name: 'yos-demo', script: 'index.js' }] };
EOF
...
EOF
assert_log_contains "start $SKILL_DIR/ecosystem.config.cjs --only yos-demo"
assert_log_not_contains "restart yos-demo"
...
const restored = fs.readFileSync('$YOS_HOME/pm2/ecosystem.config.cjs', 'utf8');
