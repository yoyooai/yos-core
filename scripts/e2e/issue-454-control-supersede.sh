...
mkdir -p "$YOS_DIR/.yos" "$YOS_DIR/activity-monitor" "$BIN_DIR"
...
FIRST_OUT="$(HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node "$CONTROL_SCRIPT" enqueue --content "dedupe me")"
SECOND_OUT="$(HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node "$CONTROL_SCRIPT" enqueue --content "dedupe me")"
...
ROOT_DIR="$ROOT_DIR" FIRST_ID="$FIRST_ID" SECOND_ID="$SECOND_ID" HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node --input-type=module <<'EOF'
...
NOW_SECS="$(date +%s)"
cat > "$YOS_DIR/activity-monitor/agent-status.json" <<EOF
{
  "state": "idle",
  "health": "ok",
  "idle_seconds": 30
...
EOF
cat > "$YOS_DIR/activity-monitor/proc-state.json" <<EOF
{
  "alive": true,
  "frozen": false,
...
THIRD_OUT="$(HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node "$CONTROL_SCRIPT" enqueue --content "dispatch me")"
FOURTH_OUT="$(HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node "$CONTROL_SCRIPT" enqueue --content "dispatch me")"
THIRD_ID="$(extract_id "$THIRD_OUT")"
FOURTH_ID="$(extract_id "$FOURTH_OUT")"

PATH="$BIN_DIR:$PATH" HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node "$DISPATCHER_SCRIPT" >"$DISPATCHER_LOG" 2>&1 &
DISPATCHER_PID=$!
sleep 2
kill -TERM "$DISPATCHER_PID" 2>/dev/null || true
wait "$DISPATCHER_PID" 2>/dev/null || true

ROOT_DIR="$ROOT_DIR" THIRD_ID="$THIRD_ID" FOURTH_ID="$FOURTH_ID" HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" node --input-type=module <<'EOF'
import assert from 'node:assert/strict';
import { pathToFileURL } from 'node:url';
