...
dump_case_artifacts() {
  ...
  for file in \
    ...
    "$YOS_DIR/activity-monitor/activity.log" \
    "$YOS_DIR/activity-monitor/agent-status.json" \
    "$YOS_DIR/activity-monitor/api-activity.json" \
    "$YOS_DIR/activity-monitor/session-tool-state.json" \
    "$YOS_DIR/activity-monitor/tool-watchdog-state.json" \
    "$YOS_DIR/activity-monitor/foreground-session.json" \
    "$YOS_DIR/activity-monitor/tool-events.jsonl"; do
    if [ -f "$file" ]; then
      echo "---- $file ----" >&2
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ~/yos/.claude/skills/activity-monitor/scripts/session-start-prompt.js"
          }
        ]
      }
    ]
...
const file = path.join(process.env.YOS_DIR, 'activity-monitor', 'agent-status.json');
...
const monitorDir = path.join(process.env.YOS_DIR, 'activity-monitor');
...
begin_case() {
  ...
  mkdir -p "$HOME_DIR" "$YOS_DIR/.yos" "$YOS_DIR/.claude" "$YOS_DIR/activity-monitor" "$YOS_DIR/comm-bridge" "$BIN_DIR"
  export HOME="$HOME_DIR"
  export YOS_DIR="$YOS_DIR"
  export YOS_PACKAGE_ROOT="$ROOT_DIR"
...
import fs from 'node:fs';
import path from 'node:path';
const file = path.join(process.env.YOS_DIR, 'activity-monitor', 'tool-events.jsonl');
...
import fs from 'node:fs';
import path from 'node:path';
const file = path.join(process.env.YOS_DIR, 'activity-monitor', 'statusline.json');
...
control_contains() {
  local pattern="$1"
  local db_path="$YOS_DIR/comm-bridge/c4.db"
...
start_monitor() {
  ROOT_DIR="$ROOT_DIR" HOME="$HOME_DIR" YOS_DIR="$YOS_DIR" YOS_PACKAGE_ROOT="$ROOT_DIR" PATH="$BIN_DIR:$PATH" TMUX_STATE_FILE="$TMUX_STATE_FILE" TMUX_LOG_FILE="$TMUX_LOG_FILE" TMUX_BUFFER_FILE="$TMUX_BUFFER_FILE" FAKE_CLAUDE_LOG="$FAKE_CLAUDE_LOG" \
  ...
  wait_until 8 "agent-status written" test -f "$YOS_DIR/activity-monitor/agent-status.json"
}
...
tc_001_escape_delivery() {
  ...
  wait_until 10 "watchdog interrupt sent" json_expr_true "$YOS_DIR/activity-monitor/tool-watchdog-state.json" 'data.interrupt_sent_at > 0'
  ...
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.watchdog_candidate_tool && data.watchdog_candidate_tool.name === "WebFetch"'
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" 'Array.isArray(data.sessions["fg-1"].running_tools) && data.sessions["fg-1"].running_tools.length === 1'
...
tc_002_normal_completion_skips_watchdog() {
  ...
  wait_until 8 "normal completion clears active tools" json_expr_true "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 0 && data.last_completed_tool && data.last_completed_tool.event_id === "toolu_ok_1" && data.last_completed_tool.status === "success"'
  if [ -f "$YOS_DIR/activity-monitor/tool-watchdog-state.json" ]; then
...
tc_003_synthetic_clear() {
  ...
  wait_until 10 "api activity cleared after prompt recovery" json_expr_true "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 0'
  if [ -f "$YOS_DIR/activity-monitor/tool-watchdog-state.json" ]; then
    echo "watchdog state should have been cleared in tc-e2e-003" >&2
    dump_case_artifacts
    exit 1
  fi
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" 'data.sessions["fg-1"].last_completed_tool && data.sessions["fg-1"].last_completed_tool.status === "cleared_by_session_event"'
  if ! tmux_log_not_contains "kill-session claude-main"; then
...
tc_004_busy_prompt_blocks_clear() {
  ...
  wait_until 10 "watchdog interrupt sent while prompt is still busy" json_expr_true "$YOS_DIR/activity-monitor/tool-watchdog-state.json" 'data.interrupt_sent_at > 0'
  sleep 1
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 1 && data.watchdog_candidate_tool && data.watchdog_candidate_tool.name === "WebFetch"'
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" 'Array.isArray(data.sessions["fg-1"].running_tools) && data.sessions["fg-1"].running_tools.length === 1'
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" '!data.sessions["fg-1"].last_completed_tool || data.sessions["fg-1"].last_completed_tool.clear_reason !== "statusline_turn_complete"'
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" '!data.sessions["fg-1"].last_completed_tool || data.sessions["fg-1"].last_completed_tool.clear_reason !== "interactive_recovered"'
}
...
tc_010_recent_statusline_does_not_override_real_completion() {
  ...
  wait_until 8 "recent statusline does not clear the active tool" json_expr_true "$YOS_DIR/activity-monitor/session-tool-state.json" 'data.sessions && data.sessions["fg-1"] && Array.isArray(data.sessions["fg-1"].running_tools) && data.sessions["fg-1"].running_tools.length === 1 && (!data.sessions["fg-1"].last_completed_tool || data.sessions["fg-1"].last_completed_tool.clear_reason !== "statusline_turn_complete")'
  send_hook_event "PostToolUse" "fg-1" "WebFetch" "$claude_pid" "$ts_done" '{"url":"https://example.com/status-race"}' "toolu_status_race"
  wait_until 8 "real completion wins over statusline hint" json_expr_true "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 0 && data.last_completed_tool && data.last_completed_tool.event_id === "toolu_status_race" && data.last_completed_tool.status === "success" && data.last_completed_tool.clear_reason !== "statusline_turn_complete"'
  if [ -f "$YOS_DIR/activity-monitor/tool-watchdog-state.json" ]; then
...
tc_007_concurrent_same_name_completions_match_by_tool_use_id() {
  ...
  wait_until 8 "two active webfetch tools visible" json_expr_true "$YOS_DIR/activity-monitor/session-tool-state.json" 'data.sessions && data.sessions["fg-1"] && Array.isArray(data.sessions["fg-1"].running_tools) && data.sessions["fg-1"].running_tools.length === 2'
  send_hook_event "PostToolUse" "fg-1" "WebFetch" "$claude_pid" "$ts_done_first" '{"url":"https://example.com/a"}' "toolu_a"
  wait_until 8 "older completion leaves newer tool active" json_expr_true "$YOS_DIR/activity-monitor/session-tool-state.json" 'data.sessions && data.sessions["fg-1"] && data.sessions["fg-1"].last_completed_tool && data.sessions["fg-1"].last_completed_tool.event_id === "toolu_a" && Array.isArray(data.sessions["fg-1"].running_tools) && data.sessions["fg-1"].running_tools.length === 1 && data.sessions["fg-1"].running_tools[0].event_id === "toolu_b"'
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.watchdog_candidate_tool && data.watchdog_candidate_tool.event_id === "toolu_b"'
  send_hook_event "PostToolUse" "fg-1" "WebFetch" "$claude_pid" "$ts_done_second" '{"url":"https://example.com/b"}' "toolu_b"
  wait_until 8 "second completion clears session" json_expr_true "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 0 && data.last_completed_tool && data.last_completed_tool.event_id === "toolu_b" && data.last_completed_tool.status === "success"'
  if ! tmux_log_not_contains "send-keys claude-main Escape"; then
...
tc_008_sequential_same_name_handoff() {
  ...
  wait_until 8 "second webfetch becomes active candidate" json_expr_true "$YOS_DIR/activity-monitor/api-activity.json" 'data.active_tools === 1 && data.watchdog_candidate_tool && data.watchdog_candidate_tool.event_id === "toolu_second" && data.last_completed_tool && data.last_completed_tool.event_id === "toolu_first"'
  wait_until 10 "sequential second tool times out" json_expr_true "$YOS_DIR/activity-monitor/tool-watchdog-state.json" 'data.episode_key === "toolu_second" && data.interrupt_sent_at > 0'
...
tc_005_escalates_to_restart() {
  ...
  wait_until 12 "watchdog escalated" json_expr_true "$YOS_DIR/activity-monitor/tool-watchdog-state.json" 'data.escalated_at > 0'
  wait_until 12 "tmux kill-session observed" tmux_log_contains "kill-session claude-main"
  assert_json_expr "$YOS_DIR/activity-monitor/agent-status.json" 'data.watchdog_phase === "escalated"'
}
...
tc_006_foreground_background_isolation() {
  ...
  sleep 4
  assert_json_expr "$YOS_DIR/activity-monitor/session-tool-state.json" 'data.sessions["fg-1"] && data.sessions["bg-1"]'
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.sessionId === "fg-1"'
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.watchdog_candidate_tool === null'
...
tc_009_old_pid_late_events() {
  ...
  wait_until 12 "old session escalated" json_expr_true "$YOS_DIR/activity-monitor/tool-watchdog-state.json" 'data.escalated_at > 0'
  ...
  wait_until 10 "new session active tool visible" json_expr_true "$YOS_DIR/activity-monitor/agent-status.json" 'data.active_tool_session_id === "fg-new"'
  ...
  assert_json_expr "$YOS_DIR/activity-monitor/agent-status.json" 'data.active_tool_session_id === "fg-new"'
  assert_json_expr "$YOS_DIR/activity-monitor/api-activity.json" 'data.sessionId === "fg-new" && data.watchdog_candidate_tool && data.watchdog_candidate_tool.name === "WebFetch"'
}
...
tc_012_launch_grace_blocks_watchdog() {
  ...
  wait_until 8 "launch grace block visible" json_expr_true "$YOS_DIR/activity-monitor/agent-status.json" 'data.watchdog_block_reason === "launch_grace"'
  sleep 4
  if control_contains "[KEYSTROKE]Escape"; then
