 ...
 *   - Persists usage data to ~/yos/activity-monitor/usage.json
...
// the deployed path — the CLI lives in the globally installed yos npm package.
// YOS_PACKAGE_ROOT is set by the PM2 ecosystem config to the installed package root.
// Dev fallback: 3 levels up from skills/activity-monitor/scripts/ = repo root (checked explicitly).
const _runtimeIndexPath = (() => {
  if (process.env.YOS_PACKAGE_ROOT) {
    return path.join(process.env.YOS_PACKAGE_ROOT, 'cli', 'lib', 'runtime', 'index.js');
  }
  ...
  // Handles the post-upgrade case where PM2 has a stale env (no YOS_PACKAGE_ROOT)
  ...
  try {
    ...
    const fallbackPath = path.join(npmRoot, 'yos', 'cli', 'lib', 'runtime', 'index.js');
    if (fs.existsSync(fallbackPath)) return fallbackPath;
  } catch { /* ignore — throw below */ }
  throw new Error(
    '[activity-monitor] Cannot locate cli/lib/runtime/index.js. ' +
    'Ensure YOS_PACKAGE_ROOT is set in the PM2 ecosystem config.'
  );
})();
...
const YOS_PATH = YOS_DIR.replace(/\//g, '-');
const CONV_DIR = path.join(os.homedir(), '.claude', 'projects', YOS_PATH);
...
// Usage monitoring config — configurable via yos config (config.json), with sensible defaults.
// Example: yos config set usage_warn_threshold 60
...
const DAILY_COMMIT_SCRIPT = path.join(__dirname, '..', '..', 'yos-memory', 'scripts', 'daily-commit.js');
