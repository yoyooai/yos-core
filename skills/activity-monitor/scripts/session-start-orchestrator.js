...
// second yos user on a multi-user host.
const SPILL_ROOT_NAME = `yos-shard-spill-${perUserSuffix()}`;
...
async function runMemoryInject(payload) {
  const { injectMemory } = await import('../../yos-memory/scripts/session-start-inject.js');
...
export function shardHeader({ position, total, name, waitNote = '' }) {
  return `=== YOS STARTUP CONTEXT [${position}/${total}] ${name}${waitNote ? ` (${waitNote})` : ''} ===`;
}
...
export async function runSessionStartShard(name, payload = {}, {
  ...
  const resolved = resolveShardImpl(name, yosDir ? { yosDir } : {});
  if (!resolved) {
