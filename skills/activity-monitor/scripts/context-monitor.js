 ...
 * - Writes status to ~/yos/activity-monitor/statusline.json for external queries
 ...
 *
 * Configured in ~/yos/.claude/settings.json:
 *   "statusLine": { "type": "command", "command": "node ~/yos/.claude/skills/activity-monitor/scripts/context-monitor.js" }
 */
...
const AM_DIR = path.join(YOS_DIR, 'activity-monitor');
...
const C4_DB = path.join(YOS_DIR, '.claude/skills/comm-bridge/scripts/c4-db.js');
const CONFIG_FILE = path.join(YOS_DIR, '.yos', 'config.json');
...
function maybeEnqueueMemorySync(usedPct) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      execFileSync('node', [C4_CONTROL, 'enqueue',
        '--content', `Context usage at ${usedPct}% (approaching ${RESTART_THRESHOLD}% session-switch threshold). Run Memory Sync now as a background task so it completes before the session switch. Launch a background subagent for memory sync following ~/yos/.claude/skills/yos-memory/SKILL.md. Do NOT wait for completion — continue normal work.`,
        '--priority', '2',
        '--no-ack-suffix'
      ], { encoding: 'utf8', stdio: 'pipe' });
