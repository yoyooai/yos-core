 ...
 * declaration files in ~/yos/.yos/shards.d/<name>.json:
 *
 *   {
 *     "name": "role-inject",            // unique shard name, [a-z0-9_-]
 *     "order": 10,                      // chain slot, components use 10+
 *     "emitter": "skills/role-manager/emit-role.js",  // under ~/yos/.claude
 ...
 * (relative to ~/yos/.claude) that hook sync may remove once the shard
...
export const CORE_SHARDS = Object.freeze([
  {
    ...
    order: 1,
    emit: async payload =>
      (await import('../../yos-memory/scripts/session-start-inject.js')).emitMemoryPart('identity', payload),
  },
  {
    // Deployment/user-provided standing directives — reads
    // ~/yos/custom-hooks/session-start/*.md at every session start. Sits right
  ...
  },
  {
    ...
    order: 3,
    emit: async payload =>
      (await import('../../yos-memory/scripts/session-start-inject.js')).emitMemoryPart('references', payload),
  },
  {
    name: 'state',
    order: 4,
    emit: async payload =>
      (await import('../../yos-memory/scripts/session-start-inject.js')).emitMemoryPart('state', payload),
  },
  {
    name: 'c4-checkpoint',
    order: 5,
...
export function defaultYOSDir(env = process.env) {
  return path.resolve(env.YOS_DIR || path.join(os.homedir(), 'yos'));
}

export function shardsDeclarationDir(yosDir = defaultYOSDir()) {
  return path.join(yosDir, '.yos', 'shards.d');
}

function normalizedClaudeRoot(yosDir) {
  return `${path.resolve(yosDir, '.claude').replaceAll('\\', '/').replace(/\/+$/, '')}/`;
}
...
function validateDeclaration(raw, { yosDir, takenNames }) {
  ...
  const claudeRoot = normalizedClaudeRoot(yosDir);
  const emitterPath = path.isAbsolute(raw.emitter)
    ? path.resolve(raw.emitter)
    : path.resolve(yosDir, '.claude', raw.emitter);
...
/**
 * Load component shard declarations from <yosDir>/.yos/shards.d/*.json.
 ...
 */
export function loadComponentShardDeclarations({ yosDir = defaultYOSDir() } = {}) {
  const dir = shardsDeclarationDir(yosDir);
  const declarations = [];
  const warnings = [];
  let files = [];
  try {
  ...
  for (const file of files) {
    ...
    const result = validateDeclaration(raw, { yosDir, takenNames });
    if (result.error) {
 ...
 */
export function buildChain({ yosDir = defaultYOSDir() } = {}) {
 ...
 */
export function resolveShard(name, { yosDir = defaultYOSDir() } = {}) {
