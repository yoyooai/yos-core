...
// The wrapper overrides YOS_DIR then spawns the actual hook
...
if (autoApprove && runtime === 'claude') {
  try {
    ...
    fs.appendFileSync(path.join(YOS_DIR, 'activity-monitor', 'enqueue-attempted.flag'), '1');
  } catch {
...
function runHook(hookData) {
  try {
    return execFileSync('node', [wrapperScript], {
      ...
      env: { ...process.env, YOS_DIR: tmpDir },
