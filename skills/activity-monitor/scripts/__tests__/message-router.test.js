...
describe('messageForRoute', () => {
  it('maps reason prefixes and health fallbacks', () => {
    ...
    const stickyRestart = messageForRoute({ health: 'unavailable', reason: 'sticky_context_restart' });
    assert.match(stickyRestart, /received.*queued/i);
    assert.match(stickyRestart, /continue automatically/i);
    assert.doesNotMatch(stickyRestart, /resend|try again/i);
