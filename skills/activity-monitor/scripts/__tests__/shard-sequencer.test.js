...
describe('shard-sequencer ladder deadlines', () => {
  ...
  it('reads T_LINK from the environment with a 1s default', () => {
    ...
    assert.equal(tLinkMs({ YOS_SHARD_T_LINK_MS: '250' }), 250);
    assert.equal(tLinkMs({ YOS_SHARD_T_LINK_MS: 'nonsense' }), DEFAULT_T_LINK_MS);
    assert.equal(tLinkMs({ YOS_SHARD_T_LINK_MS: '-5' }), DEFAULT_T_LINK_MS);
  });
});
...
describe('shard-sequencer flag chain', () => {
  ...
  it('keys the flag root per user so multi-user hosts cannot collide on a shared /tmp root', () => {
    ...
    assert.equal(flagRoot({ tmpdir }), path.join(tmpdir, `yos-shard-flags-${suffix}`));
  });
});
...
describe('shard-sequencer same-session re-trigger (compact keeps session_id)', () => {
  ...
  it('reads the freshness tolerance from the environment with a sane default', () => {
    ...
    assert.equal(flagFreshToleranceMs({ YOS_SHARD_FLAG_FRESH_TOLERANCE_MS: '250' }), 250);
    assert.equal(flagFreshToleranceMs({ YOS_SHARD_FLAG_FRESH_TOLERANCE_MS: '0' }), 0);
    assert.equal(flagFreshToleranceMs({ YOS_SHARD_FLAG_FRESH_TOLERANCE_MS: 'nonsense' }), DEFAULT_FLAG_FRESH_TOLERANCE_MS);
    assert.equal(flagFreshToleranceMs({ YOS_SHARD_FLAG_FRESH_TOLERANCE_MS: '-5' }), DEFAULT_FLAG_FRESH_TOLERANCE_MS);
