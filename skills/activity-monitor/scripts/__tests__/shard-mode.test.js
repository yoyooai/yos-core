...
function writeDeclaration(yosDir, fileName, declaration) {
  const dir = path.join(yosDir, '.yos', 'shards.d');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, fileName), JSON.stringify(declaration, null, 2));
}

function makeComponentYOSDir() {
  const yosDir = makeTmpdir('shard-yos-');
  ...
  writeDeclaration(yosDir, 'role-inject.json', {
    name: 'role-inject',
    order: 10,
...
describe('shard-registry chain', () => {
  it('builds the 6 core shards in the agreed order', () => {
    const { chain } = buildChain({ yosDir: makeTmpdir() });
    assert.deepEqual(
  ...
  it('appends declared component shards after the core shards', () => {
    const yosDir = makeComponentYOSDir();
    const { chain, warnings } = buildChain({ yosDir });
  ...
  it('rejects invalid declarations without breaking the chain', () => {
    ...
    writeDeclaration(yosDir, 'reserved.json', { name: 'identity', order: 10, emitter: 'skills/x/e.js' });
    writeDeclaration(yosDir, 'low-order.json', { name: 'too-early', order: 2, emitter: 'skills/x/e.js' });
    writeDeclaration(yosDir, 'escape.json', { name: 'escape', order: 11, emitter: '/etc/evil.js' });
    writeDeclaration(yosDir, 'bad-claim.json', {
      name: 'bad-claim', order: 12, emitter: 'skills/x/e.js', claimHooks: ['/custom/user-hook.js'],
    });
    fs.writeFileSync(path.join(yosDir, '.yos', 'shards.d', 'garbage.json'), '{nope');

    const { declarations, warnings } = loadComponentShardDeclarations({ yosDir });
  ...
  it('rejects duplicate component shard names (first declaration wins)', () => {
    ...
    writeDeclaration(yosDir, 'a.json', { name: 'dup', order: 10, emitter: 'skills/a/e.js' });
    writeDeclaration(yosDir, 'b.json', { name: 'dup', order: 11, emitter: 'skills/b/e.js' });
  ...
  it('clamps component budgets to the dual-limit ceiling', () => {
    const yosDir = makeTmpdir();
    writeDeclaration(yosDir, 'big.json', {
      name: 'big', order: 10, emitter: 'skills/x/e.js', budget: { maxChars: 99999, maxTokens: 99999 },
    });
    const { declarations } = loadComponentShardDeclarations({ yosDir });
...
describe('runSessionStartShard (content shards)', () => {
  ...
  it('emits the numbered header and body, then registers its completion flag for exit time', async () => {
    ...
    assert.equal(out.read(), '=== YOS STARTUP CONTEXT [1/2] identity ===\nIDENTITY BODY\n');
  ...
  it('waits for the predecessor flag before emitting', async () => {
    ...
    assert.equal(out.read(), '=== YOS STARTUP CONTEXT [2/2] references ===\nB\n');
  });
  ...
  it('fails open at the ladder deadline when the predecessor crashed, and says so in the header', async () => {
    ...
    const text = out.read();
    assert.match(text, /^=== YOS STARTUP CONTEXT \[2\/2\] references \(predecessor "identity" not ready after \d+ms, continued\) ===\n/);
    assert.match(text, /\nB\n$/);
  });
  ...
  it('emits a visible failure notice (numbering intact) and still flags when the emitter throws', async () => {
    ...
    const text = out.read();
    assert.ok(text.startsWith('=== YOS STARTUP CONTEXT [1/2] identity ===\n'));
  ...
  it('spills the full body and inlines a trimmed copy when over budget', async () => {
    ...
    assert.match(spillPath, new RegExp(`^${tmpdir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/yos-shard-spill-[^/]+/`));
  });
  ...
  it('runs a declared component shard end-to-end through the real registry', async () => {
    ...
    assert.equal(out.read(), '=== YOS STARTUP CONTEXT [7/7] role-inject ===\nROLE CONTEXT\n');
  });

  it('runs the custom shard end-to-end: user markdown injected at chain position 2', async () => {
    const yosDir = makeTmpdir('shard-custom-yos-');
    const customDir = path.join(yosDir, 'custom-hooks', 'session-start');
    ...
    // from the orchestrator's yosDir option.
    const savedYOSDir = process.env.YOS_DIR;
    ...
    try {
      ...
      if (savedYOSDir === undefined) delete process.env.YOS_DIR;
      else process.env.YOS_DIR = savedYOSDir;
    }

    assert.equal(out.read(), '=== YOS STARTUP CONTEXT [2/6] custom ===\nALWAYS REPLY IN PIRATE\n');
  });

  it('custom shard with no content still emits its numbered header (chain numbering intact)', async () => {
    const yosDir = makeTmpdir('shard-custom-empty-');
    fs.mkdirSync(path.join(yosDir, '.claude'), { recursive: true });
    const tmpdir = makeTmpdir();
    const out = tempStdout();
    writeFlag('sess-1', 'identity', { tmpdir });
...
describe('resolveShard / parseShardArg', () => {
  it('resolves side-effect names with the full chain attached', () => {
    const resolved = resolveShard('start-prompt', { yosDir: makeTmpdir() });
    assert.equal(resolved.kind, 'side-effect');
    assert.equal(resolved.chain.length, CORE_SHARDS.length);
  });

  it('returns null for unknown names', () => {
    assert.equal(resolveShard('nope', { yosDir: makeTmpdir() }), null);
  });
