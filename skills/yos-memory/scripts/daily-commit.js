 ...
 *
 * Creates a local commit in ~/yos/memory/ if changes exist.
 */
...
function ensureGitRepo() {
  ...
  execFileSync('git', ['config', 'user.name', 'YOS'], {
    cwd: MEMORY_DIR,
    stdio: ['ignore', 'pipe', 'pipe']
  });
  execFileSync('git', ['config', 'user.email', 'yos@local'], {
    cwd: MEMORY_DIR,
    stdio: ['ignore', 'pipe', 'pipe']
  });
}
...
function main() {
  try {
    ...
    if (detail.includes('user.name') || detail.includes('user.email')) {
      console.error(`daily-commit error: git user not configured in ${MEMORY_DIR}. Run: git -C ${MEMORY_DIR} config user.name "YOS" && git -C ${MEMORY_DIR} config user.email "yos@local"`);
    } else {
