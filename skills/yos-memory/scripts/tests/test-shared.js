...
import { dateInTimeZone, BUDGETS, WARN_THRESHOLDS, YOS_DIR, MEMORY_DIR, SESSIONS_DIR } from '../shared.js';
...
describe('directory constants', () => {
  it('YOS_DIR defaults to ~/yos when YOS_DIR env is not set', () => {
    // The constant is evaluated at import time using process.env.YOS_DIR
    // We just verify the shape is correct
    assert.equal(typeof YOS_DIR, 'string');
    assert.ok(YOS_DIR.length > 0);
  });

  it('MEMORY_DIR is YOS_DIR/memory', () => {
    assert.equal(MEMORY_DIR, path.join(YOS_DIR, 'memory'));
  });
