...
const SKILLS_DIR = process.env.WEB_CONSOLE_SKILLS_DIR || path.join(os.homedir(), 'yos', '.claude', 'skills');
const DB_DIR = path.join(YOS_DIR, 'comm-bridge');
const DB_PATH = path.join(DB_DIR, 'c4.db');
const STATUS_FILE = path.join(YOS_DIR, 'activity-monitor', 'agent-status.json');
const MEDIA_DIR = path.join(YOS_DIR, 'web-console', 'media');
...
function readEnvPassword() {
  return ENV.YOS_WEB_PASSWORD || ENV.WEB_CONSOLE_PASSWORD || '';
}
...
try {
  ...
  if (!tableCheck) {
    ...
    console.error('Run: node ~/yos/.claude/skills/comm-bridge/scripts/c4-db.js init');
    process.exit(1);
  }
} catch (err) {
...
app.get('/api/media/:messageId', (req, res) => {
  try {
    ...
    const allowedPath = resolveAllowedPathSync(result.media.path, [YOS_DIR, '/tmp']);
    if (!allowedPath) {
