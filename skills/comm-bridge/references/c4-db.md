...
```bash
~/yos/.claude/skills/comm-bridge/scripts/c4-db.js <command> [args]
```
...
[
  {
    ...
    "source_message_id": "update_123456",
...
| `insertConversation()` | Queue a new message; atomically returns the existing row for duplicate channel/source-message IDs |
| `setConversationDeliveryAction(id, action)` | Record non-content delivery metadata such as status-notice suppression |
