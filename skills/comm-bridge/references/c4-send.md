...
```bash
cat <<'EOF' | node ~/yos/.claude/skills/comm-bridge/scripts/c4-send.js <channel> <endpoint_id>
...
cat <<'EOF' | node ~/yos/.claude/skills/comm-bridge/scripts/c4-send.js telegram
Hello everyone!
EOF
```
...
```bash
cat <<'C4MSG' | node ~/yos/.claude/skills/comm-bridge/scripts/c4-send.js telegram 8101553026
...
```bash
cat <<'EOF' | node ~/yos/.claude/skills/comm-bridge/scripts/c4-send.js "void" "session-handoff"
...
Channels are skills installed in `~/yos/.claude/skills/`. Each channel must provide:

- **Send script**: `~/yos/.claude/skills/<channel>/scripts/send.js <endpoint_id> <message>`
- **Config**: `~/yos/<channel>/config.json` (for data like `primary_dm`)
