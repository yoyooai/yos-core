...
Control messages are stored in the `control_queue` table in `~/yos/comm-bridge/c4.db`.

## Usage

```bash
~/yos/.claude/skills/comm-bridge/scripts/c4-control.js <enqueue|get|ack> [options]
```
...
~/yos/.claude/skills/comm-bridge/scripts/c4-control.js \
    enqueue --content "Heartbeat check." \
    --ack-deadline 120

# Check status of control 5
~/yos/.claude/skills/comm-bridge/scripts/c4-control.js get --id 5

# Acknowledge control 5
~/yos/.claude/skills/comm-bridge/scripts/c4-control.js ack --id 5
