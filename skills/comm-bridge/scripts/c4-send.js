...
async function main() {
  ...
  if (cleanArgs.length === 2 && (stdinAvailable || hasStdinFlag)) {
    // Non-interactive parents commonly expose an already-closed stdin pipe.
    // Only treat the second argument as an endpoint when stdin contains a
    // message (or --stdin explicitly requests that form).
    const stdinMessage = (await readStdin()).trimEnd();
    if (stdinMessage || hasStdinFlag) {
