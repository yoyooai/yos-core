...
function _readConfig() {
  try {
    return JSON.parse(fs.readFileSync(path.join(YOS_DIR, '.yos', 'config.json'), 'utf8'));
  } catch {
    return {};
  }
}
...
export const DATA_DIR = path.join(YOS_DIR, 'comm-bridge');
export const DB_PATH = path.join(DATA_DIR, 'c4.db');
export const ACTIVITY_MONITOR_DIR = path.join(YOS_DIR, 'activity-monitor');
