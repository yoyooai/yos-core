...
import fs from 'node:fs';
import path from 'node:path';
const outPath = path.join(process.env.YOS_DIR, '${channel}-send.json');
...
import fs from 'node:fs';
import path from 'node:path';
const outPath = path.join(process.env.YOS_DIR, '${channel}-send.jsonl');
...
describe('c4-receive basic intake', () => {
  ...
  it('deduplicates repeated delivery by explicit channel message id', () => {
    withTmpDir(({ tmpDir, env }) => {
      const args = [
        '--no-reply',
        '--channel', 'system',
      ...
      const second = cliRaw(args, env);

      assert.equal(first.status, 0);
      assert.equal(second.status, 0);
      const firstOut = parseJsonStdout(first.stdout);
      const secondOut = parseJsonStdout(second.stdout);
      assert.equal(firstOut.action, 'queued');
      assert.equal(secondOut.action, 'duplicate');
      assert.equal(secondOut.id, firstOut.id);

      const db = openDb(tmpDir);
      const rows = db.prepare("SELECT source_message_id, content FROM conversations WHERE direction = 'in'").all();
      db.close();
      assert.deepEqual(rows, [{ source_message_id: 'event-123', content: 'run this once' }]);
    });
  });

  it('derives a channel message id from an endpoint msg segment', () => {
    withTmpDir(({ tmpDir, env }) => {
      createChannelSendScript(tmpDir, 'test-chan');
      const args = [
        '--channel', 'test-chan',
        '--endpoint', 'chat-1|type:group|msg:om_123',
        '--json',
        '--content', 'endpoint dedupe'
      ];
      const first = cliRaw(args, env);
      const second = cliRaw(args, env);
      ...
      assert.equal(first.status, 0);
      assert.equal(second.status, 0);
      assert.equal(parseJsonStdout(second.stdout).action, 'duplicate');

      const db = openDb(tmpDir);
      const row = db.prepare("SELECT source_message_id FROM conversations WHERE direction = 'in'").get();
      db.close();
      assert.equal(row.source_message_id, 'om_123');
    });
  });
...
describe('c4-receive validation', () => {
  ...
  it('errors when --message-id has no value', () => {
    withTmpDir(({ env }) => {
      const r = cliRaw(['--no-reply', '--message-id', '--json', '--content', 'missing id'], env);
      assert.equal(r.status, 1);
      const out = parseJsonStdout(r.stdout);
      assert.equal(out.ok, false);
      assert.equal(out.error.code, 'INVALID_ARGS');
      assert.match(out.error.message, /message-id.*value/i);
    });
  });
...
describe('c4-receive health gating', () => {
  ...
  it('durably queues unhealthy messages for automatic recovery delivery', () => {
    withTmpDir(({ tmpDir, env }) => {
      ...
      assert.equal(inbound.content, 'recover this work');

      const sent = JSON.parse(fs.readFileSync(path.join(tmpDir, 'test-chan-send.json'), 'utf8'));
      assert.match(sent.args[1], /received.*queued|queued.*recover/i);
      assert.doesNotMatch(sent.args[1], /resend|send your message again|try again/i);
    });
  });
  ...
  it('fallback keeps no-reply unhealthy messages pending without error', () => {
    withTmpDir(({ tmpDir, env }) => {
  ...
  it('keeps the message queued if unhealthy status delivery fails', () => {
    withTmpDir(({ tmpDir, env }) => {
      fs.writeFileSync(path.join(tmpDir, 'activity-monitor', 'agent-status.json'), JSON.stringify({ health: 'down' }));
      ...
      db.close();
      assert.equal(inbound.status, 'pending');
      assert.equal(inbound.content, 'down msg');
      assert.match(r.stderr, /failed to send unhealthy status message/i);
    });
  });
  ...
  it('fallback suppresses repeated status notices by normalized endpoint and public health', () => {
    withTmpDir(({ tmpDir, env }) => {
      ...
      assert.equal(first.status, 0);
      assert.equal(parseJsonStdout(first.stdout).action, 'queued');
      ...
      assert.equal(suppressed.content, 'second');
      assert.equal(suppressed.delivery_action, 'status_suppressed');
    });
  });
  ...
  it('fallback reserves status notice cooldown before sending so concurrent receives suppress duplicates', async () => {
    await withTmpDirAsync(async ({ tmpDir, env }) => {
      ...
      assert.deepEqual(actions, ['queued', 'queued']);
      ...
      const inboundCount = db.prepare("SELECT count(*) AS count FROM conversations WHERE direction = 'in'").get();
      const outboundCount = db.prepare("SELECT count(*) AS count FROM conversations WHERE direction = 'out'").get();
      const originalContentCount = db.prepare("SELECT count(*) AS count FROM conversations WHERE content IN ('first concurrent', 'second concurrent')").get();
      const suppressedActionCount = db.prepare("SELECT count(*) AS count FROM conversations WHERE delivery_action = 'status_suppressed'").get();
      db.close();

      assert.equal(inboundCount.count, 2);
      assert.equal(outboundCount.count, 1);
      assert.equal(originalContentCount.count, 2);
