...
after(() => {
  if (origYOSDir === undefined) {
    delete process.env.YOS_DIR;
  } else {
    process.env.YOS_DIR = origYOSDir;
  }
...
describe('findPromptY', () => {
  it('finds › prompt line in a Codex-style capture', () => {
    const capture = [
      ...
      '› hello world',                                     // Y=3
      '',                                                  // Y=4
      '  gpt-5.4 medium · 82% left · ~/yos'             // Y=5
    ].join('\n');
  ...
  it('returns last prompt line when multiple exist', () => {
    const capture = [
      ...
      '› second prompt',                                   // Y=2
      '',                                                  // Y=3
      '  gpt-5.4 medium · 82% left · ~/yos'             // Y=4
    ].join('\n');
  ...
  it('finds empty prompt line (just › with no text)', () => {
    const capture = [
      ...
      '›',                                                 // Y=1
      '',                                                  // Y=2
      '  gpt-5.4 medium · 82% left · ~/yos'             // Y=3
    ].join('\n');
    assert.equal(findPromptY(capture), 1);
  });
  ...
  it('finds prompt when Codex output contains ─ separator lines', () => {
    ...
    const capture = [
      ...
      '› user input',                                      // Y=4
      '',                                                  // Y=5
      '  gpt-5.4 medium · 82% left · ~/yos'             // Y=6
    ].join('\n');
