...
if (ORIG_YOS_DIR === undefined) delete process.env.YOS_DIR;
else process.env.YOS_DIR = ORIG_YOS_DIR;
...
describe('insertConversation', () => {
  ...
  it('deduplicates source message ids within a channel', () => {
    const first = mod.insertConversation('in', 'telegram', '123', 'hello', null, 3, false, null, 'msg-1');
    const duplicate = mod.insertConversation('in', 'telegram', '123', 'hello again', null, 3, false, null, 'msg-1');

    assert.equal(first.duplicate, false);
    assert.equal(duplicate.duplicate, true);
    assert.equal(duplicate.id, first.id);
    const rows = db.prepare("SELECT content, source_message_id FROM conversations WHERE direction = 'in'").all();
    assert.deepEqual(rows, [{ content: 'hello', source_message_id: 'msg-1' }]);
  });

  it('allows the same source message id on different channels', () => {
    const telegram = mod.insertConversation('in', 'telegram', '123', 'one', null, 3, false, null, 'msg-1');
    const lark = mod.insertConversation('in', 'lark', '456', 'two', null, 3, false, null, 'msg-1');

    assert.equal(telegram.duplicate, false);
    assert.equal(lark.duplicate, false);
    assert.notEqual(lark.id, telegram.id);
  });
});
