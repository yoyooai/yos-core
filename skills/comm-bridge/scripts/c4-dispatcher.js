...
export function buildDeliveryFailureAlert({ conversationId, channel, endpoint, retryCount }) {
  return [
    '[YOS] Inbound message delivery failed',
    `Conversation ID: ${conversationId}`,
    `Source channel: ${channel || 'unknown'}`,
    `Source endpoint: ${endpoint || 'none'}`,
    `Retry count: ${retryCount}`,
    'The message remains recorded locally with status=failed. User content is omitted from this alert.'
  ].join('\n');
}
...
export function notifyAdministratorOfDeliveryFailure(payload, {
  ...
  const endpoint = env.YOS_ADMIN_ENDPOINT?.trim();
  if (!channel || !endpoint) {
    log(`ALERT NOT SENT: delivery failure for conversation id=${payload.conversationId}; configure YOS_ADMIN_CHANNEL and YOS_ADMIN_ENDPOINT`);
    return { sent: false, reason: 'not_configured' };
  }

  const result = runSend('node', [path.join(__dirname, 'c4-send.js'), channel, endpoint], {
    input: buildDeliveryFailureAlert(payload),
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
  if (result.status !== 0) {
    const detail = String(result.stderr || result.stdout || `exit ${result.status}`).trim();
    log(`ALERT SEND FAILED: conversation id=${payload.conversationId} (${detail})`);
    return { sent: false, reason: 'send_failed', detail };
  }

  log(`Administrator alerted for failed conversation id=${payload.conversationId}`);
  return { sent: true };
}

export async function handleConversationDeliveryFailure(msg, {
  channelHealthy = isAgentStatusFresh(),
  notifyAdmin = notifyAdministratorOfDeliveryFailure,
  wait = sleep
} = {}) {
    ...
    if (nextCount >= MAX_RETRIES) {
      ...
      try {
        await notifyAdmin({
          ...
          channel: msg.channel,
          endpoint: msg.endpoint_id || null,
          retryCount: nextCount
        });
      } catch (err) {
        log(`ALERT SEND FAILED: conversation id=${msg.id} (${err.message})`);
      }
      return;
    }
