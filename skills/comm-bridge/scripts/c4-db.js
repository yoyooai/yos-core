...
function ensureConversationsSchema(database) {
  ...
  if (!columnNames.has('source_message_id')) {
    database.exec('ALTER TABLE conversations ADD COLUMN source_message_id TEXT');
  }
  database.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_conversations_channel_source_message
      ON conversations(channel, source_message_id)
 ...
 */
export function insertConversation(direction, channel, endpointId, content, status = null, priority = 3, requireIdle = false, deliveryAction = null, sourceMessageId = null) {
  const db = getDb();
  ...
  const stmt = db.prepare(`
    INSERT OR IGNORE INTO conversations (
      direction, channel, endpoint_id, source_message_id, content,
      status, delivery_action, priority, require_idle
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  if (result.changes === 0) {
    const existing = sourceMessageId === null ? null : db.prepare(`
      SELECT id, direction, channel, endpoint_id, source_message_id, content,
             status, delivery_action, priority, require_idle, retry_count
      FROM conversations
      WHERE channel = ? AND source_message_id = ?
    `).get(channel, sourceMessageId);
    if (!existing) {
      throw new Error('conversation insert was ignored without a matching source message');
    }
...
export function setConversationDeliveryAction(id, deliveryAction) {
  const database = getDb();
  database.prepare('UPDATE conversations SET delivery_action = ? WHERE id = ?').run(deliveryAction, id);
}
