...
```bash
node -e "try { const c=JSON.parse(require('fs').readFileSync(require('path').join(process.env.HOME,'yos/.yos/config.json'),'utf8')); console.log(c.runtime||'claude'); } catch { console.log('claude'); }"
```
...
```bash
cat ~/yos/activity-monitor/statusline.json
```
