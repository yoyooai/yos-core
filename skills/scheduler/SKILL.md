...
1. Activity Monitor writes state to `~/yos/activity-monitor/agent-status.json`
...
`~/yos/.claude/skills/scheduler/scripts/cli.js <command>`
...
Resolves TZ: `~/yos/.env` > `process.env.TZ` > `UTC`. Times are parsed and displayed in configured timezone. DB stores UTC. See `references/config.md` for details.
