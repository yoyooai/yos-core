...
const YOS_DIR = process.env.YOS_DIR || join(homedir(), 'yos');
const STATUS_FILE = join(YOS_DIR, 'activity-monitor', 'agent-status.json');

/**
 * Read agent status from ~/yos/activity-monitor/agent-status.json
...
function findC4ReceivePath() {
  ...
  const productionPath = join(homedir(), 'yos/.claude/skills/comm-bridge/scripts/c4-receive.js');
