...
```bash
node ~/yos/.claude/skills/<component>/hooks/pre-upgrade.js
```

Set environment variables: `YOS_COMPONENT`, `YOS_SKILL_DIR`, `YOS_DATA_DIR`.
...
```bash
node ~/yos/.claude/skills/<component>/hooks/post-upgrade.js
```
...
2. Run `yos upgrade telegram --yes --skip-eval --json`
