...
2. The **local version** is backed up to `$YOS_DIR/.backup/<timestamp>/conflicts/<skill>/<path>` for self-upgrade, or `<skillDir>/.backup/<timestamp>/conflicts/<path>` for component upgrade
...
In CLI mode (`yos upgrade`), conflicts are reported with backup paths. The user is prompted:
...
```bash
yos upgrade <component> --check --json
```
...
```bash
yos upgrade <component> --yes --skip-eval --json
```
...
1. **Post-upgrade hook**: read the `post_upgrade_hook` step in the `steps` array. Post-upgrade hooks never report `failed`. Treat a `skipped` result with `no post-upgrade hook` as normal. Investigate every other `skipped` result, including `hook had issues`, `hook not found`, and `hook path escapes skill directory`, because the declared config migration did not complete.
...
## Session Mode — Self-Upgrade (yos-core)

### Step 1: Check + Download to Temp

```bash
yos upgrade --self --check --json
```
...
Run `yos upgrade telegram --check --json`, parse the JSON output, then actively analyze what changed:

1. Read the `changelog` field from JSON output (if present)
2. Fetch commit history from the component's recorded repository: `gh api repos/<owner>/<repository>/compare/v<current>...v<latest> --jq '.commits[].commit.message'` (with proxy)
...
2. Verify the `post_upgrade_hook` step result. Post-upgrade hooks never report `failed`. Treat a `skipped` result with `no post-upgrade hook` as normal. Investigate every other `skipped` result, including `hook had issues`, `hook not found`, and `hook path escapes skill directory`, because the declared config migration did not complete.
...
1. **Post-upgrade hook**: check the `post_upgrade_hook` step result. Post-upgrade hooks never report `failed`. Treat a `skipped` result with `no post-upgrade hook` as normal. Investigate every other `skipped` result, including `hook had issues`, `hook not found`, and `hook path escapes skill directory`, because the declared config migration did not complete.
...
## C4 Mode — Self-Upgrade (yos-core)

Same two-step pattern for upgrading YOS itself. **All information shown before confirmation.**

### Step 1 — User requests

User: `upgrade yos`

Run `yos upgrade --self --check --json`. Then:

1. **Analyze changes**: Read changelog + fetch commit history from `YOS_RELEASE_REPO` via `gh api repos/<owner>/<repository>/compare/v<current>...v<latest> --jq '.commits[].commit.message'` (with proxy). Synthesize change summary.
...
If already up to date, reply: `YOS core is up to date (v0.1.0)`

### Step 2 — User confirms

User: `upgrade yos confirm`

1. Run `yos upgrade --self --yes --json`
