...
```bash
printf '%s\n' '{"API_KEY":"value"}' | YOS_COMPONENT=<component> YOS_SKILL_DIR=~/yos/.claude/skills/<component> YOS_DATA_DIR=~/yos/components/<component> node ~/yos/.claude/skills/<component>/hooks/configure.js
```
...
- If no configure hook exists, write collected values to `~/yos/.env` for legacy components.

### Step 4: Set Up PATH for Bin Commands

If the JSON output includes `skill.bin` entries (component provides CLI commands), `~/yos/bin` must be in PATH. Since each shell command runs in a separate process, **prefix all subsequent commands** that use component CLIs with the PATH export:

```bash
export PATH="$HOME/yos/bin:$PATH" && <command>
```
...
```bash
export PATH="$HOME/yos/bin:$PATH" && node ~/yos/.claude/skills/<component>/hooks/post-install.js
```
...
After `yos add <name> --json` succeeds, the JSON `skill` field tells you what to do:

1. **Bin PATH**: If `skill.bin` exists, prefix all subsequent commands with `export PATH="$HOME/yos/bin:$PATH" && `.
2. **Config**: If `skill.config.required` exists, inform user which config values are needed. In C4 mode, user provides values via follow-up messages.
   - If `skill.hooks.configure` exists, pipe collected values as stdin JSON to the configure hook with `YOS_COMPONENT`, `YOS_SKILL_DIR`, and `YOS_DATA_DIR` set.
   ...
   - If no configure hook exists, write collected values to `~/yos/.env` for backward compatibility.
