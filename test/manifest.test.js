...
describe('generateManifest', () => {
  ...
  test('excludes .yos, node_modules, .git, .backup directories', () => {
    const dir = mkTmp();
    writeFile(dir, 'a.js', 'content');
    writeFile(dir, '.yos/manifest.json', '{}');
...
describe('saveMergeBaseline', () => {
  test('commits manifest and originals together on success', () => {
    ...
    expect(fs.existsSync(path.join(component, '.yos', 'originals.bak'))).toBe(false);
  });
  ...
  test('originals staging failure leaves the live baseline pair untouched', () => {
    ...
    const manifestBefore = fs.readFileSync(path.join(component, '.yos', 'manifest.json'), 'utf8');
    ...
    expect(fs.readFileSync(path.join(component, '.yos', 'manifest.json'), 'utf8')).toBe(manifestBefore);
    expect(getOriginalContent(component, 'a.js')).toBe('v1');
    expect(fs.existsSync(path.join(component, '.yos', 'manifest.json.tmp'))).toBe(false);
    expect(fs.existsSync(path.join(component, '.yos', 'originals.new'))).toBe(false);
  });
...
describe('recoverMergeBaseline', () => {
  ...
  test('uncommitted staged transaction rolls back — idempotently', () => {
    ...
    writeFile(component, '.yos/manifest.json.tmp', '{"files":{"bogus.js":"dead"}}');
    writeFile(component, '.yos/originals.new/bogus.js', 'staged junk');
  ...
  test('rollback interrupted between its two deletions cannot surface the uncommitted staging', () => {
    ...
    writeFile(component, '.yos/manifest.json.tmp', '{"files":{"a.js":"deadbeef"}}');
    writeFile(component, '.yos/originals.new/a.js', 'v2-staged-uncommitted');
  ...
  test('committed transaction with interrupted swap rolls forward — idempotently', () => {
    ...
    writeFile(component, '.yos/originals.new/a.js', 'v2');
  ...
  test('legacy bak with matching live originals is dropped — local file edits do not trigger recovery', () => {
    ...
    writeFile(component, '.yos/originals.bak/a.js', 'v0');
  ...
  test('legacy bak matching the manifest is restored when live originals are broken', () => {
    ...
    writeFile(component, '.yos/originals.bak/a.js', 'v1');
    writeFile(component, '.yos/originals/a.js', 'partial garbage');
  ...
  test('legacy bak where neither side matches: explicit error, site preserved', () => {
    ...
    writeFile(component, '.yos/originals/a.js', 'garbage-active');
    writeFile(component, '.yos/originals.bak/a.js', 'garbage-bak');

    expect(() => recoverMergeBaseline(component)).toThrow(/manual inspection/);
    // Nothing deleted — both candidates preserved for inspection
    expect(fs.readFileSync(path.join(component, '.yos', 'originals', 'a.js'), 'utf8')).toBe('garbage-active');
    expect(fs.readFileSync(path.join(component, '.yos', 'originals.bak', 'a.js'), 'utf8')).toBe('garbage-bak');
  });

  test('no-op on a clean baseline and on a missing .yos dir', () => {
    const clean = mkTmp();
    setupBaseline(clean);
    const manifestBefore = fs.readFileSync(path.join(clean, '.yos', 'manifest.json'), 'utf8');
    recoverMergeBaseline(clean);
    expect(fs.readFileSync(path.join(clean, '.yos', 'manifest.json'), 'utf8')).toBe(manifestBefore);
