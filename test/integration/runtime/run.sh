...
IMAGE_NAME="${Z_RUNTIME_IMAGE:-yos-runtime-test:local}"
REAL_IMAGE_NAME="${Z_RUNTIME_REAL_IMAGE:-yos-runtime-test:real}"
...
# For real-smoke scenarios that run `claude -p` (not just `yos runtime`), the
...
run_one() {
  ...
  local run_dir
  run_dir="$(mktemp -d "${TMPDIR:-/tmp}/yos-runtime-${scenario}.XXXXXX")"
  ...
  local docker_args=(
    ...
    -e "HOME=/tmp/yos-home"
    -e "YOS_DIR=/tmp/yos-home/yos"
    -e "GOLDEN_DIR=/opt/yos-golden"
    -e "FAKE_INVOCATION_LOG=/tmp/yos-run/invocations.log"
    -v "$run_dir:/tmp/yos-run"
  )
  ...
  #   init    — clone the golden, build-time `yos init` workspace baked into
  #             the image at $GOLDEN_DIR, giving a real post-init state.
  # YOS_DIR is deliberately $HOME/yos (see docker run below): generated
  # artifacts such as pm2/ecosystem.config.cjs resolve skill paths from
  # $HOME/yos, NOT from $YOS_DIR, so a post-init workspace is only usable
  # when this production invariant (YOS_DIR == $HOME/yos) holds.
  # SCENARIO_CMD is any command line (typically a `yos ...` invocation).
  local container_script='
set -uo pipefail
mkdir -p "$HOME" "$YOS_DIR/.yos" "$YOS_DIR/pm2"
...
if [ -n "${SETUP_RUNTIME:-}" ]; then
  printf "{\"runtime\":\"%s\"}\n" "$SETUP_RUNTIME" > "$YOS_DIR/.yos/config.json"
fi

if [ -n "${SCENARIO_ENV_FILE:-}" ]; then
  printf "%b\n" "$SCENARIO_ENV_FILE" > "$YOS_DIR/.env"
elif [ "${SETUP:-minimal}" = minimal ] && [ ! -f "$YOS_DIR/.env" ]; then
  : > "$YOS_DIR/.env"
fi
