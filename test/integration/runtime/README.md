...
generic "run any `yos` command in a controlled container and assert on the
...
`run.sh all` is the regression entry point. It builds `yos-runtime-test:local`
...
passes, it builds a separate `yos-runtime-test:real` image with the genuine
...
- The base image installs yos from the local source tree.
...
- Each scenario runs with an isolated `HOME`, and `YOS_DIR` is set to
  `$HOME/yos` — the same invariant production relies on. This matters for
  `SETUP=init`: generated artifacts such as `pm2/ecosystem.config.cjs` resolve
  skill paths from `$HOME/yos`, not from `$YOS_DIR`, so a cloned post-init
...
- `minimal` (default) — empty workspace. Optionally seed `SETUP_RUNTIME` to
  write `.yos/config.json` (e.g. set the *current* runtime so a runtime switch
  is a real switch, not an already-on-target no-op).
- `init` — clone the **golden init workspace** baked into the image. At build
  time the Dockerfile runs `yos init --yes --quiet` once (with
  `YOS_DIR == $HOME/yos`, the production invariant) into `/opt/yos-golden`;
  `SETUP=init` clones that into the scenario's `YOS_DIR` (which is `$HOME/yos`).
...
```text
SCENARIO_CMD=yos runtime claude
EXPECT_EXIT=0
```
...
line — `yos runtime claude --no-validate`, `yos runtime status`,
`yos doctor`, etc. `EXPECT_INVOCATIONS` / `EXPECT_INVOCATION_CONTAINS` assert
...
- `claude-*`, `codex-login-status-fail` — runtime auth fail-closed regression
  cases (minimal setup). These permanently guard the `yos runtime` auth
  invariants.
- `post-init-runtime-status` — demonstrates `SETUP=init`: runs
  `yos runtime status` against the golden post-init workspace and asserts it
