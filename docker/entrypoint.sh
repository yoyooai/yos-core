#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────
# YOS Docker Entrypoint
#
# 1. Validates required auth environment variables
# 2. Runs yos init (creates/updates .env and workspace — every startup)
...
YOS_DIR="${HOME}/yos"
ENV_FILE="${YOS_DIR}/.env"

# ── Colour helpers ────────────────────────────────────────────────────────────
info()  { echo -e "\033[0;36m[yos]\033[0m $*"; }
ok()    { echo -e "\033[0;32m[yos]\033[0m ✓ $*"; }
warn()  { echo -e "\033[1;33m[yos]\033[0m $*"; }
error() { echo -e "\033[0;31m[YOS]\033[0m $*" >&2; }
step()  { echo -e "\033[0;36m[yos]\033[0m ── Step $1/$TOTAL_STEPS: $2"; }

TOTAL_STEPS=4
YOS_VERSION="$(yos --version 2>/dev/null || echo 'dev')"

echo ""
info "=========================================="
info "  YOS ${YOS_VERSION}"
info "=========================================="
echo ""
...
# ── Step 2: Workspace initialisation via yos init ───────────────────────────
# yos init handles: directory structure, .env creation from template,
...
if ! yos init ${INIT_ARGS}; then
  warn "yos init exited with errors (may be partial). Check logs."
fi

ok "Workspace ready"

# ── Pass through channel env vars to .env ─────────────────────────────────────
# yos init doesn't write channel tokens — those come from component installs.
...
pm2 start "${YOS_DIR}/pm2/ecosystem.config.cjs" --no-daemon &
PM2_PID=$!
...
# Determine runtime: YOS_RUNTIME env var always wins; fall back to config.json.
if [ -z "${YOS_RUNTIME:-}" ]; then
  YOS_RUNTIME=$(node -e "
    try {
...
if [ "${YOS_RUNTIME}" = "codex" ]; then
  ...
  tmux new-session -d -s codex-main -x 220 -y 50 \
    "cd ${YOS_DIR} && source ${ENV_FILE} 2>/dev/null; exec codex ${CODEX_ARGS}"
  ...
  tmux new-session -d -s claude-main -x 220 -y 50 \
    "cd ${YOS_DIR} && source ${ENV_FILE} 2>/dev/null; exec claude ${CLAUDE_ARGS}"
