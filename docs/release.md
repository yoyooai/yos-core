...
5. **校准过程文档。** 放行单、装机作业单、工单模板这类**过程文档是快照** ——
   写下那一刻是对的，之后债被修好了它们**不会自己变绿**。本轮凡是随本版闭环的债，
   **回头看一眼这些文档里还有没有在按它的旧病形教人排查**，有就当轮改掉或标注作废。

   **判据：一个没参与本轮的人照着这些文档做，会不会走冤枉路。**

   > 2026-08-15 真踩过：对客装机解禁时，我照 08-12 立的放行单（红项写于 08-13）
   > 向苏白报了**两条阻断项**，而那两条 08-14 都已随 `0.1.17` 修好出货
   > —— 报的内容是"劝他别装"，**差点让对客交付整个推迟**。
   > 同一次还查出装机作业单有**五处**在描述已经修好的病。
   > ⇒ **病根是把过程文档当状态源。** 现状只有两个来源：**债务台账的 PAID 状态**，和**现查**。
   > 单子上任何一条红项，都要标注核对时刻；标不出来的，就是没核过。
