...
> **没有 node、没有两个代码仓**，它现在只是一台供静态文件的 nginx —— 它能供货，造不了货。
> 本轮改成的分工，比原来那套更干净，建议保持：
>
> | 机器 | 做什么 |
> |---|---|
> | **控制机** | 构建、切换前自审、旧货架自审出凭据、站外上传与恢复演练（长期密钥本来就只在这里） |
> | **货架机** | 只做两件事：接收整棵树、**原子换软链** |
>
> **跨机交接的判据是整树指纹，不是"传完了没报错"**：两侧各跑一次
> `find . -type f | LC_ALL=C sort | xargs sha256sum | sha256sum`，**逐字相同才准切换**。
> （0.1.19 实测 `c660c1f1…` 两侧一致，1121 文件、0 软链。）
>
> 要恢复货架机的构建能力＝重装 node ＋ 克隆两仓，**属于动生产机，需授权**。
...
> 🔴🔴 **2026-08-27 起，下面的命令有一个前提已经不成立了，先读这段再照做。**
>
> 本节 ④⑤ 以及【控制机】那段末尾 `ssh` 过去执行的部分，**都要求货架机上有 node 和本仓库**。
> **香港货架机 08-26 清空重装后两样都没有**（见第 3 步开头那段）。
>
> **⇒ 货架机没有 node 时，改这样做（0.1.19 出货实际走的就是这条路，全程实测通过）**：
>
> | 子步骤 | 原文说在哪 | 没有 node 时在哪 | 怎么做 |
> |---|---|---|---|
> | ⓿①②③ | 货架机 | **仍在货架机** | 原文不变（只用 `readlink`/`cp`/`tar`/`sha256sum`，不需要 node） |
> | — | — | **新增一步** | 把 ② 的 tar 包拉到控制机：`ssh <货架机> 'sudo cat /var/backups/yos-dist-$OLD-$STAMP.tar.gz' > <本地路径>`，**核 sha256 与货架机记的逐字相同**，再解开 |
> | ④⑤ | 货架机 | **控制机** | 对解开的那棵树跑 `--local --full --json`，命令与原文逐字相同，只是 `--local` 后面换成本地路径 |
> | 站外 upload/verify | 控制机拿钥匙 → ssh 到货架机执行 | **全在控制机** | 直接对本地那棵树跑 `shelf-offsite.mjs`，不再 `ssh`；钥匙照旧不落盘 |
>
> **这个分工其实比原文更干净**：长期密钥、构建、自审、上传全在控制机，生产机只做原子换软链。
> 而且**恢复演练本来就要求在非货架机上做** —— 现在自审也在非货架机上，货架机上那条
> 「顺着符号链接读到真货架、假 PASS」的坑从根上没有了。
>
> ⚠️ **下面的原文没有改写，是有意的**：它对「货架机有 node」的机器仍然正确，
> 而改写它要连带改动 `test/release-doc.test.js` **真去执行**的那几个命令块 ——
> 那是该单独走一轮验收的改动，不是发布当晚顺手做的。已记 TD-274。
...
#    先把清单落成文件再看，不要 `tar … | head`：head 提前关管道会让 tar 收 SIGPIPE
#    退出码 141，而执行人几乎一定加了 set -o pipefail —— 于是脚本在这里中止，
#    现象是「备份自审失败、凭据文件不存在」，看起来像备份坏了。
#    2026-08-14 与 2026-08-27 连踩两次，第二次是把第一次的教训写进台账之后踩的。
TARLIST=/var/backups/yos-dist-$OLD-$STAMP.tar.list
tar -tzf /var/backups/yos-dist-$OLD-$STAMP.tar.gz > "$TARLIST"
wc -l < "$TARLIST"        # 应是几百条以上
head -3 "$TARLIST"        # 第一条必须是目录，不是 ->
...
> **✅ 2026-08-27（`0.1.19` 出货）首次真的执行了这一套，三条前置条件当场逐条核过、全部成立：**
> `disable_symlinks` 与 `open_file_cache` 在 `/etc/nginx/` 全域**均未配置**（默认 off）；
> `sites-available/yos-dist` 的 `root /srv/yos-dist` **指的是那根符号链接本身**，不是它当时指向的真目录。
> 换链接切换实测无中间态，切换后公网签字 1120/1120 PASS。
>
> ⚠️ **但这三条要每次核，不是一次核完就永远成立** —— 它们是那台机器的 nginx 配置属性，
> 谁改了配置就会变。（2026-08-14 也核过一次，核的是**当时门户站**的配置；
> 08-26 清机后货架搬了家，配置文件换了一个，那次的结论对今天**不适用**。）
> 旧的「整目录 `mv` 换入」做法仍可用，只是它有上面说的那个窗口。
...
### 第 11 步 · 结账（**没做完这一步，这次发布不算完**）

前十步交付的是**货**；这一步交付的是**账**。跳过它，下一个人（包括你自己）
拿到的是一份说谎的现状 —— 而现状说谎比没有现状更坏，因为它不会让人起疑。

**这一步没有命令，只有三件必须当轮做完的事**（不许留到"以后补"）：

1. **回填内部发布记录。** 本次的版本号、`buildId`、`index.json` 摘要、包哈希、文件数、
   标签数、备份的 RUN 前缀与对象数、回退命令、同机保留物、待清项 —— 逐项落到
   内部发布记录里。**判据：出事那天有人只读这一份文件，能不能独立把回退做完。**

2. **把这次发出去的债销掉。** 债务台账里凡是随本版上了货架的条目，
   **标题与正文 `Status` 两处都要改**，不能只改一处 ——
   看板遇到两边打架时按【未解决】计（宁可高报欠债），只改一处等于没改。
   口径见 TD-58：**合进主干 ≠ 客户手上有**，只有上了货架才算闭环。

3. **过一遍对外问题清单。** 已随本版解决的整条移走，
   受本版影响而结论变了的就地改写。
   **不能只更新数字** —— 要逐条问"这里描述的东西现在还在不在"。
   数字过期只是不准；描述的东西整个不存在了还挂在榜上，那是假账。

4. **清掉过期的那一层备份。** 回退链**只需要两层**：当前版本，和它的上一版
   （＝真正的回退目标）。再往前的那一层，在本版终验通过的这一刻就到期了 ——
   货架机上那一版的真目录与 `bak-` 副本、`/var/backups/` 里对应的压缩包与凭据，
   本轮一并清掉。**当前版与上一版的东西一个字节都不许动。**

   **构建目录与 vendor 缓存另算：只留当前版。** 它们**不在回退链上**
   —— 回退取的是货架副本，不是构建产物；构建目录随时可以从 tag 重建。
   上一版的那一份在本版构建完成的那一刻就没有用途了。

   恢复演练拉下来的那份（第 5 步③ 的 `restore-*`）**演练结束即删** ——
   它的全部价值在"验过了"这个结论上，结论已经写进本轮发布记录。

   > 手册此前只写了「留到下一版终验通过后再清」，**没写"终验后要清"** ——
   > 于是从来没人清。2026-08-13 一次清出 **18 份、1.8G**，最早一份来自 `0.1.2`。
   > 与"测试建了临时目录不删"是同一族病：**只管生不管埋**。
   > 删之前先 `readlink -f` 看清楚目标是什么、多大、属于谁；**照记忆里的路径直接删是禁止的。**

> 🔴 **这一步是补出来的，补之前它发作过七次。** 从 2026-08-09 到 2026-08-15，
> 「汇报时账比货旧」这个形状**连续复发七次**，每次的复盘结论都是同一句
> "下次记得"，于是每次都有下一次。**第七次（`0.1.18` 出货当天）仍然复发** ——
> 那天台账在汇报前回填了，问题清单是汇报后才想起来的。
> ⇒ 病根从来不是记性，**是这份手册里没有这一格**。现在有了。
> 判据很硬：**这三件没做完，就不要去汇报「发完了」。**
