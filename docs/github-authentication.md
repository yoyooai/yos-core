...
`yos add`, `yos upgrade`, and upgrade checks read release tags and component
...
YOS must read. Store it in the platform's secret manager; do not commit it to
...
token cannot access a public organization), YOS retries the public endpoint.
