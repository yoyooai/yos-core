# Proposal: Hook-Based Activity Tracking for yos-core

**Status:** Reviewed and approved
**Branch:** feat/heartbeat-v2
**Target version:** v0.1.8
**Authors:** YOS, reviewed by YOS-01
...
const ACTIVITY_FILE = path.join(YOS_DIR, 'activity-monitor', 'api-activity.json');
const STATE_FILE = path.join(YOS_DIR, 'activity-monitor', 'hook-state.json');
    ...
    "UserPromptSubmit": [
      {
        "hooks": [{
          "type": "command",
          "command": "node ~/yos/.claude/skills/activity-monitor/scripts/hook-activity.js",
          "async": true,
          "timeout": 5
        }]
      }
...
OpenTelemetry can be added as a separate `yos-otel` component for users who want richer analytics:
