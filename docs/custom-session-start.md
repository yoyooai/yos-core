...
- **Where it lands:** as the numbered shard right after the identity shard (`=== YOS STARTUP CONTEXT [2/N] custom ===`), so custom directives frame everything that follows (references, state, conversation context).
...
| Cross-deployment behavior shipped to every install | `YOS.md` core template |
...
- Do not duplicate content that already lives in `memory/` or YOS.md — the directory should hold only directives that genuinely need always-on injection.

## Example

```bash
mkdir -p ~/yos/custom-hooks/session-start
cat > ~/yos/custom-hooks/session-start/10-go-toolchain.md <<'EOF'
