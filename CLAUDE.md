...
Runtime data (logs, databases, config) goes in `~/yos/<skill-name>/`, NOT in the skill source directory.
...
This keeps code (in `skills/`) separate from runtime data (in `~/yos/`).

## PM2 Process Management

### Ecosystem Configuration

**All PM2 services MUST be managed through `~/yos/pm2/ecosystem.config.cjs`.**
...
2. **Restart all services** - `pm2 delete all && pm2 start ~/yos/pm2/ecosystem.config.cjs`
